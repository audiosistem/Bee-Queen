...
class Collections:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.luc_kodi/?action=collections&url=%s,return)' % (quote_plus(url)))
...
####################################
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False):
		try:
			...
			url = 'plugin://plugin.video.luc_kodi/?action=%s' % action
			cm = []
