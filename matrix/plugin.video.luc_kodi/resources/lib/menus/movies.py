...
class Movies:
	def __init__(self, notifications=True):
		...
		# Si el usuario tiene FanartTV desactivado pero tiene pantalla 4K detectada, se activa
		# automáticamente para aprovechar el arte adicional en alta resolución.
		...
		self.traktwatchedweekly_link = 'https://api.trakt.tv/movies/watched/weekly?limit=%s&page=1' % self.page_limit
		self.traktwatchedmonthly_link = 'https://api.trakt.tv/movies/watched/monthly?limit=%s&page=1' % self.page_limit
	...
	def unfinishedManager(self):
		try:
			...
			del window
			if selected_items:
				refresh = 'plugin.video.luc_kodi' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.luc_kodi/?action=movies&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.luc_kodi/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
	...
	def watchproviders(self):
		use_jw = (getSetting('streaming.use_justwatch') == 'true')
		jw_map = {}
		if use_jw:
			try:
				from resources.lib.indexers.justwatch import TMDB_TO_JW_PACKAGE
				jw_map = TMDB_TO_JW_PACKAGE
			except: use_jw = False
		watchproviders = tmdb_indexer().get_watchproviders()
		for i in watchproviders:
			if use_jw:
				# i[1] can be '337' or '337|390' — take the first id for the JW lookup
				try: tmdb_pid = int(str(i[1]).split('|')[0])
				except: tmdb_pid = None
				jw_code = jw_map.get(tmdb_pid) if tmdb_pid else None
	...
	def getJustWatch(self, jw_package_code, create_directory=True):
		self.list = []
		try:
			self.list = tmdb_indexer().jw_list(jw_package_code)
			if self.list is None: self.list = []
			if create_directory: self.movieDirectory(self.list)
			return self.list
		except:
	...
	def worker(self):
		try:
			...
			self.list = [i for i in self.list if i.get('tmdb') or i.get('imdb')]
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
	...
	def addDirectory(self, items, queue=False):
		...
		for i in items:
			try:
				...
				# ── Bingie modern grid: si el perfil 'bingie' está activo, los
				#    items de submenú que apuntan a listados de pelis se abren en
				#    la ventana propia. ──
				try:
					from resources.lib.modules import views
					if views.getViewStyle() == 'bingie':
						_a = i['action']
						_tmdb = '&tmdb=1' if _a in ('tmdbmovies', 'tmdbmoviePage') else ''
						if _a in ('movies', 'moviePage', 'tmdbmovies', 'tmdbmoviePage'):
							url = url.replace('?action=%s' % _a, '?action=bingie_gridMovies', 1)
							url += '&title=%s%s' % (quote_plus(name), _tmdb)
				...
				item.addContextMenuItems(cm)
				_isFolder = False if 'action=bingie_grid' in url else True
				control.addItem(handle=syshandle, url=url, listitem=item, isFolder=_isFolder)
			except:
				from resources.lib.modules import log_utils
				log_utils.error()
		try:
