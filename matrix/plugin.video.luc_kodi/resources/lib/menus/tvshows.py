...
class TVshows:
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.luc_kodi/?action=tvshows&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.luc_kodi/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
	...
	def worker(self):
		try:
			...
			self.list = [i for i in self.list if i.get('tmdb') or i.get('tvdb')]
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
...
####-Context Menu and Overlays-####
				...
				if fb_tmdb:
					fb_mt     = quote_plus(i.get('_fb_media_type', 'tv'))
					fb_genres = quote_plus(i.get('_fb_genres', ''))
					fb_title  = quote_plus(i.get('tvshowtitle') or i.get('title', ''))
	...
	def addDirectory(self, items, queue=False):
		...
		for i in items:
			try:
				...
				# ── Bingie modern grid (perfil 'bingie'): items de submenú que
				#    apuntan a listados de series se abren en la ventana propia. ──
				try:
					from resources.lib.modules import views
					if views.getViewStyle() == 'bingie':
						_a = i['action']
						_tmdb = '&tmdb=1' if _a in ('tmdbTvshows', 'tmdbTvshowPage') else ''
						if _a in ('tvshows', 'tvshowPage', 'tmdbTvshows', 'tmdbTvshowPage'):
							url = url.replace('?action=%s' % _a, '?action=bingie_gridTVShows', 1)
