...
#				url += "&auth_token=%s" % self.token
			...
			# RD May-2026 keyword filter: HTTP 451 + error_code 35.
			# Capture status BEFORE .json() so caller can react explicitly and
			# we don't spam the user with a generic toast for a known filter hit.
			_rd2026_hit = (getattr(response, 'status_code', 0) == 451)
			if 'Temporarily Down For Maintenance' in response.text:
				if self.server_notifications: control.notification(message='Real-Debrid Temporarily Down For Maintenance', icon=rd_icon)
				log_utils.log('Real-Debrid Temporarily Down For Maintenance', level=log_utils.LOGWARNING)
				return None
			else: response = response.json()
			if _rd2026_hit or (isinstance(response, dict) and response.get('error_code') == 35):
				log_utils.log(
					'Real-Debrid RD2026 filter: infringing_file (HTTP 451 / error_code 35) on %s' % original_url,
			...
			if any(value in str(response) for value in ('bad_token', 'Bad Request')):
				...
				if message == 'infringing_file':
					# Same condition as above, surfaced through the 'error'
					# key shape instead of the status code. Don't toast.
					log_utils.log(
						'Real-Debrid RD2026 filter: infringing_file on %s' % original_url,
						__name__, log_utils.LOGWARNING)
					return None
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def play_cached(self, magnet_url, info_hash, title=None, season=None, episode=None):
		"""Convenience wrapper to resolve and play only cached torrents via Real-Debrid.
		Returns a direct stream URL, or None on failure.
		"""
		try:
			# Reuse the proven resolver which already enforces cached-only selection
			return self.resolve_magnet(magnet_url, info_hash, season, episode, title or '')
		except:
			from resources.lib.modules import log_utils
			log_utils.error('RD.play_cached failed')
			return None


	def list_cached(self, hashes):
		"""Return Real-Debrid instant availability map for an iterable of info hashes.
		Each hash should be a hex string; returns dict keyed by lowercase hash.
		"""
		try:
			return self.check_cache([h.lower() for h in hashes])
		except:
			from resources.lib.modules import log_utils
			log_utils.error('RD.list_cached failed')
			return {}
