...
#			cloud_folders = [i for i in cloud_folders if i['status'] == 'downloaded']
	...
	def show_and_play_cached(self, magnet_url, info_hash, season=None, episode=None, title=''):
		"""Display (if a pack) and resolve a cached torrent to a direct stream URL via RD.
		This relies on RealDebrid.resolve_magnet which already filters out uncached and
		prompts file selection for packs when appropriate.
		"""
		try:
			return RealDebrid().resolve_magnet(magnet_url, info_hash, season, episode, title)
		except:
			from resources.lib.modules import log_utils
			log_utils.error('RD_CLOUD: show_and_play_cached failed')
			return None
