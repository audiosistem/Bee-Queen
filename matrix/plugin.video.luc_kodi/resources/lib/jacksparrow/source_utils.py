...
from resources.lib.jacksparrow import cleantitle
from resources.lib.jacksparrow.undesirables import Undesirables
from resources.lib.jacksparrow.control import homeWindow, setting as getSetting, setSetting
...
def get_qual(term):
	# Order matters: high → low. RES_720 contains '720' (a generic 3-char
	# substring) and would otherwise match years like '1720' or fragments
	# inside '2160p' release tags. Checking 4K → 1080 → 720 is the only
	# safe order with substring matching.
...
def get_extra_tags(name):
	"""Append codec/HDR tags that require the raw release name (not name_info).
	Must be called with clean_name() output — info_from_name() strips '+' and
	other chars that are needed to detect e.g. HDR10+ and AV1.
	Returns a list of tag strings to merge into the scraper info list."""
	tags = []
	if not name:
		return tags
	n = name.lower()
	...
	if '.hevc.' in n or '.x265.' in n or '.h265.' in n:
		tags.append('HEVC')
	# H264 / x264
	elif '.x264.' in n or '.h264.' in n or '.avc.' in n:
		tags.append('H264')
...
def filter_season_pack(show_title, aliases, year, season, release_title):
	...
	try:
		...
		season = str(season)  # guard int→str (next_ep_dialog pasa season como int)
...
def info_from_name(release_title, title, year, hdlr=None, episode_title=None, season=None, pack=None):
	try:
		...
		if pack:
			if pack == 'season':
				season = str(season)  # guard int→str
				season_fill = season.zfill(2)
