...
addonObject = addon('plugin.video.luc_kodi') # addonObject = addon('script.module.jacksparrowscrapers')
...
cacheFile = joinPath(dataPath, 'jacksparrowcache.db')
undesirablescacheFile = joinPath(dataPath, 'jacksparrowundesirables.db')
settingsFile = joinPath(dataPath, 'settings.xml')


def setting(id, fallback=None):
#	try: settings_dict = jsloads(homeWindow.getProperty('jacksparrowscrapers_settings'))
...
#		homeWindow.setProperty('jacksparrowscrapers_settings', jsdumps(settings_dict))
		homeWindow.setProperty('luc_kodi_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear jacksparrowscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('jacksparrowscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('jacksparrowscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.jacksparrowscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('jacksparrowScrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.jacksparrowscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.jacksparrowscrapers').getAddonInfo('version')
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.jacksparrowscrapers'
	try:
		removed_settings = []
		active_settings = []
...
def notification(title=None, message=None, icon=None, time=3000, sound=False):
	"""
	Plugin-wide notification entry point used by the jacksparrow framework
	(scrapers, sootio_wizard, undesirables cache, log_utils, etc.).

	Delegates to the unified luc_kodi notification (modules.control.notification)
	so EVERY notification — regardless of origin module — renders in the same
	top-center phone-style XML window. This used to call xbmcgui.Dialog().notification()
	directly, which Kodi always pins to the bottom-right corner; the delegation
	makes notification placement consistent across the whole plugin.

	The original native call is kept as a defensive fallback in case the unified
	path is unavailable (e.g. import error during a startup race or partial install).
	"""
	try:
		# Lazy import to avoid any startup-time ordering issues between
		# the jacksparrow framework and the luc_kodi modules layer.
		from resources.lib.modules import control as _luc_control
		_luc_control.notification(title=title, message=message, icon=icon, time=time, sound=sound)
		return
	except Exception:
		pass  # fall through to native fallback below

	# --- Defensive fallback (native Kodi toast, bottom-right) ----------------
