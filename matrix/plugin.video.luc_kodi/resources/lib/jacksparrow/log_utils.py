...
import inspect
from resources.lib.jacksparrow.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ jacksparrowSCRAPERS %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'jacksparrowscrapers.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.jacksparrowonscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
#			addon = 'script.module.jacksparrowscrapers'
			addon = 'plugin.video.luc_kodi'
		...
		import xbmc
		xbmc.log('[ script.module.jacksparrowonscrapers ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from resources.lib.jacksparrow.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.jacksparrowonscrapers ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared

def view_LogFile(name):
	try:
		from resources.lib.jacksparrow.textviewer import TextViewerXML
		from resources.lib.jacksparrow.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from resources.lib.jacksparrow.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from resources.lib.jacksparrow.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		...
		if 'key' in response.json():
			...
			log('jacksparrowScrapers log file uploaded to: %s' % result)
			...
			if 'Copy url To Clipboard' in list[select][0]:
				from resources.lib.jacksparrow.source_utils import copy2clip
				copy2clip(list[select - 1][1])
		elif 'message' in response.json():
			notification(message='jacksparrowScrapers Log upload failed: %s' % str(response.json()['message']))
			log('jacksparrowScrapers Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='jacksparrowScrapers Log upload failed')
			log('jacksparrowScrapers Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('jacksparrowScrapers log upload failed')
