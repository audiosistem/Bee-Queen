...
"""
	jacksparrowscrapers Project - Meteor scraper

	Meteor es un Stremio addon propietario desarrollado y operado en
	exclusiva por @midnightignite (verificado en stremio-addons.net/addons/
	meteor-midnightignite, "Developer: midnightignite"). A diferencia de
	Torrentio, Comet o MediaFusion — que son proyectos open-source con
	multiples deployments publicos — Meteor solo existe en una unica
	instancia: meteorfortheweebs.midnightignite.me. No hay forks ni
	espejos publicos.

	IMPLICACION OPERACIONAL: si esa instancia cae, este scraper devuelve
	cero resultados hasta que vuelva. El plugin no tiene fallback porque
	no hay otro endpoint compatible con el contrato Stremio /stream/.

	POLITICA DEFAULT (desde v1.0.18): provider.meteor = false en
	settings.xml. Razonamiento: Meteor entrega un subset de fuentes que
	en gran parte coinciden con Torrentio + Torrentio-Spanish (ambos
	activos por default). Mantener Meteor activo por default haria que
	una caida del operador unico introdujese latencia/timeouts en cada
	busqueda sin aportar fuentes que no estuvieran ya cubiertas. Los
	usuarios que quieran Meteor pueden activarlo manualmente.
"""
