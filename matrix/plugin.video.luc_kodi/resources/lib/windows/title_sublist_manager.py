...
class TitleSublistManagerXML(BaseDialog):
		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					org_id = chosen_listitem.getProperty('luc_kodi.originalTitle')
					if chosen_listitem.getProperty('luc_kodi.isSelected') == 'true':
						...
						original_title = chosen_listitem.getProperty('luc_kodi.originalTitle')
						sub_title = chosen_listitem.getProperty('luc_kodi.subTitle')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('luc_kodi.originalTitle', item.get('originalTitle'))
					listitem.setProperty('luc_kodi.subTitle', str(item.get('subTitle')))
	...
	def set_properties(self):
		try:
			...
			self.setProperty('luc_kodi.highlight.color', self.highlight_color)
			self.setProperty('luc_kodi.luc_kodi_icon', joinPath(artPath(), 'icon.png'))
		except:
			from resources.lib.modules import log_utils
			log_utils.error()