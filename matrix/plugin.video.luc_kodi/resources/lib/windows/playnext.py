...
class PlayNextXML(BaseDialog):
	...
	def onClick(self, control_id):
		if control_id == 3011: # Play Now — BigSkipForward ends E01, Kodi advances playlist to E02 with valid handle
	...
	def background_tasks(self):
		try:
			...
			if not file_changed:
				if self.default_action == 0:    # Play Next — BigSkipForward or already ending naturally
					xbmc.executebuiltin('PlayerControl(BigSkipForward)')
				elif self.default_action == 1:  # Stop
					xbmc.executebuiltin('PlayerControl(Playlist.Clear)')
					xbmc.executebuiltin('PlayerControl(Stop)')
				elif self.default_action == 2:  # Pause
