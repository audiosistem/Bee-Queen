...
def cache_clear_search():
	...
	try:
		...
		dbcur = dbcon.cursor()
		for t in ('movies', 'tvshow', 'collections', 'furk', 'easynews', 'ai_movies', 'ai_tvshow', 'ai_auto'):
...
def get_connection_bookmarks():
	...
	return conn
##################
def clear_local_bookmarks(): # clear all luc_kodi bookmarks from kodi database
	try:
		dbcon = db.connect(get_video_database_path())
		dbcur = dbcon.cursor()
		# The Kodi video DB may not have the 'files' table yet (fresh install or
		# library never used). That's not an error — just nothing to clear.
		dbcur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='files'")
		if not dbcur.fetchone():
			return
		dbcur.execute('''SELECT * FROM files WHERE strFilename LIKE "%plugin.video.luc_kodi%"''')
...
def clrCache_version_update(clr_providers=False, clr_metacache=False, clr_cache=False, clr_search=False, clr_bookmarks=False):
	try:
		...
		control.log('[ plugin.video.luc_kodi ]  Forced cache clear for version update complete.', 1)
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def update_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('luc_kodi Addon Data Path Does not Exist. Creating Folder....', __name__, log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.luc_kodi')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = control.addon('plugin.video.luc_kodi').getAddonInfo('version')
