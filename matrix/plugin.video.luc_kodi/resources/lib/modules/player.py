...
def _detect_mime(url):
	"""Return a MIME type hint for a stream URL so Kodi does not re-probe on seek."""
	try:
		clean = (url or '').lower().split('?')[0].split('|')[0]
		...
		pass
	return 'video/x-matroska'  # mkv is the most common container from Debrid


def _add_stream_headers(url):
	"""No-op pass-through.

	Previous versions appended `Accept-Ranges=bytes&Connection=keep-alive` to
	HTTPS Debrid URLs, but: (a) Accept-Ranges is a *response* header — sending
	it as a request is meaningless and some strict servers reject it; (b)
	Connection=keep-alive is already the HTTP/1.1 default. Real seek
	optimisation is achieved by setContentLookup(False) + setMimeType() in
	play_source(), so this helper is kept only for call-site compatibility.
	"""
	return url
...
class Player(xbmc.Player):
	def __init__(self):
		...
		self.ended_naturally = False  # set by onPlayBackEnded → always show next-ep dialog
		self.playnext_time = 60  # fixed — playnext.time setting removed in v0.0.92
...
##################
			...
			# ── Stream optimizations (instant seek / no re-probe) ──────────────
			url = _add_stream_headers(url)
			# ───────────────────────────────────────────────────────────────────
			...
			_next_ep_direct = homeWindow.getProperty('luc_kodi.next_ep_direct') == 'true'
			_bingie_direct = homeWindow.getProperty('luc_kodi.bingie_direct') == 'true'
			# NO borrar next_ep_direct aquí — onAVStarted lo borrará cuando el video arranque (evita FALLBACK doble búsqueda)
			if debridPackCall or _next_ep_direct or _bingie_direct:
				# debridPack, next-episode-dialog y Bingie UI usan player.play() directo.
				# Bingie lanza la reproducción con RunPlugin desde una ventana custom,
				# por lo que argv[1] es un handle inválido y setResolvedUrl() no haría nada.
				homeWindow.clearProperty('luc_kodi.bingie_direct')
				control.player.play(url, item)
			else:
				control.resolve(int(argv[1]), True, item, self.meta)
	...
	def keepAlive(self):
		...
		if playlist_skip: pass
			...
			# ── POST-STOP: Next Episode On-Demand Dialog ──────────────────────────────
			# Conditions to show the dialog:
			#   A) video ended naturally (onPlayBackEnded fired)          — always eligible
			#   B) user stopped manually after >=85% watched             — eligible
			#   C) stream had no duration (media_length=0) but ran >60s  — eligible (can't compute %)
			_watched_ok = (
				self.ended_naturally                                          # A
				or self.getWatchedPercent() >= 85                            # B
				or (self.media_length == 0 and int(self.current_time) > 60)  # C: stream sin duración
			)
			if (self.media_type == 'episode'
					and self.enable_playnext
					and _watched_ok
	...
	def _show_next_episode_dialog(self):
		...
		Muestra el diálogo, y si el usuario confirma llama directamente a
		Sources().play() en el mismo hilo — sin RunPlugin (que usa handle=-1 y
		...
		try:
			...
			# v1.0.18 FIX: strip per-episode fields BUT preserve 'title' as the show
			# title. Previously 'title' was popped entirely, leaving meta['title']=None,
			# which propagated to sources.sourcesResolve -> torbox.resolve_magnet ->
			# title.lower() AttributeError -> every E02 magnet failed -> "no streams".
			# Other debrids (RD/AD/PM) don't call title.lower() so they didn't expose
			# this. We don't know the next episode's real title yet, so we use the
			# show title as a safe placeholder — it's what the URL `&title=` arg
			# already passes downstream.
			for _k in ('thumb', 'plot', 'premiered', 'landscape'):
				next_meta.pop(_k, None)
			next_meta['title'] = show_title

			ep_label = 'S%02dE%02d' % (next_season, next_ep)

			from resources.lib.windows.next_episode_dialog import NextEpisodeDialog
			# Pasar landscape/thumb del episodio actual como contexto visual del diálogo.
			# self.meta tiene los datos del ep que acaba de terminar — el landscape
			# es el still del episodio actual, útil como fondo del card en 2160p.
			_ep_thumb = (self.meta or {}).get('landscape', '') or (self.meta or {}).get('thumb', '')
			dialog = NextEpisodeDialog(
				'next_episode_dialog.xml',
				control.addonPath(control.addonId()),
				show_title=show_title,
				ep_label=ep_label,
				fanart=next_meta.get('fanart', ''),
				clearlogo=next_meta.get('clearlogo', ''),
				thumb=_ep_thumb,
			)
			ret = dialog.run()
			del dialog
			# ret: 1=Auto-Play, 2=Choose Source, 0=Not Now/timeout
			if ret not in (1, 2):
				return

			select = '1' if ret == 1 else '0'

			# Usar RunPlugin con handle=-1 es correcto para lanzar sin bloquear.
			# play_source() detecta next_ep_direct=true y usa control.player.play()
			# en vez de setResolvedUrl, evitando el error de handle inválido.
			# La playlist se limpia aquí primero para evitar "skipping unplayable item".
			from json import dumps as jsdumps
			from urllib.parse import quote_plus
			control.playlist.clear()  # limpiar playlist antes de lanzar siguiente ep
			homeWindow.setProperty('luc_kodi.next_ep_direct', 'true')
			sysmeta  = quote_plus(jsdumps(next_meta))
			systitle = quote_plus(show_title)
			plugin_url = (
				'plugin://plugin.video.luc_kodi/?action=play_Item'
				'&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s'
				'&season=%s&episode=%s&tvshowtitle=%s&premiered=&meta=%s&select=%s'
			) % (
				systitle, self.year or '',
				self.imdb or '', self.tmdb or '', self.tvdb or '',
				next_season, next_ep, systitle, sysmeta, select
			)
	...
	def _queue_next_ep_if_needed(self):
		...
		try:
			...
			next_meta['episode']   = str(next_ep)
			next_meta['mediatype'] = 'episode'
			# v1.0.18 FIX: see _show_next_episode_dialog — don't pop 'title' (was leaving
			# meta['title']=None which broke torbox.resolve_magnet). Use show title.
			for _k in ('thumb', 'plot', 'premiered', 'landscape'):
				next_meta.pop(_k, None)
			next_meta['title'] = self.title or ''
...
### Kodi player callback methods ###
	def onAVStarted(self): # Kodi docs suggests "Use onAVStarted() instead of onPlayBackStarted() as of v18"
		# Limpiar el flag next_ep_direct ahora que el video arrancó de verdad.
		# No lo limpiamos antes para que el FALLBACK del ep anterior no dispare una 2ª búsqueda.
		...
		if self.offset != '0' and self.playback_resumed is False:
			...
			# seekTime() needs a real number. Bookmarks().get() returns the offset
			# as a STRING, and unless the trakt-resume branch above converted it,
			# passing a str raised "TypeError: must be real number, not str" — the
			# seek failed and playback always restarted from the beginning. Coerce
			# to float defensively so resume works in every path.
			try:
				_seek = float(self.offset)
...
class Subtitles:
	...
	def __init__(self):
		...
		self.playnext_method = '0'  # fixed — playnext.method setting removed in v0.0.92
	...
	def get(self, title, year, imdb, season, episode):
		...
		# ── Global guard: prevent simultaneous subtitle lookups across ALL
		# callers (keepAlive, onAVStarted, SubtitlePlayer) which run in
		# different threads/processes but share homeWindow properties.
		_FLAG = 'luc_kodi.subtitles.running'
		if homeWindow.getProperty(_FLAG) == 'true':
			control.log('[ luc_kodi ] Subtitles.get() — already running, skipping duplicate call', LOGINFO)
			return
		homeWindow.setProperty(_FLAG, 'true')
...
class PlayNext(xbmc.Player):
	def __init__(self):
		...
		self.enable_playnext = getSetting('enable.playnext') == 'true'
		self.stillwatching_count = 3  # fixed — stillwatching.count setting removed in v0.0.92
...
class Bookmarks:
	...
	def set_scrobble(self, current_time, media_length, media_type, imdb='', tmdb='', tvdb='', season='', episode=''):
		try:
			...
			# - <80% but >3 min played = /scrobble/pause (saves resume point)
			# - else (very short interrupt) = no call
			# IMPORTANT: don't fire both pause AND stop on the same set_scrobble
			# call — the 5s local cooldown would suppress whichever runs second,
			# leading to a watched item being saved as merely a resume point.
			if getSetting('simkl.scrobble') == 'true':
				try:
					from resources.lib.modules import simkl as _simkl
					if _simkl.getSimklCredentialsInfo():
						if percent >= 80:
							_simkl.scrobbleStop(imdb=imdb, tmdb=tmdb, tvdb=tvdb,
