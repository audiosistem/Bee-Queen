...
DEBUGPREFIX = '[COLOR red][ luc_kodi: %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'luc_kodi.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ plugin.video.luc_kodi ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.luc_kodi ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
...
def clear_logFile():
	...
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.luc_kodi ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		windows = TextViewerXML('textviewer.xml', addonPath('plugin.video.luc_kodi'), heading=heading, text=text)
		windows.run()
		del windows
	except: error()
...
def upload_LogFile(name):
	...
	try:
		...
		if 'key' in response.json():
			...
			select = selectDialog([i[0] for i in list], lang(32196) if name.lower() == 'luc_kodi' else lang(32199))
