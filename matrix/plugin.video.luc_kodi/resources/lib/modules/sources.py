...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		...
		try:
			...
			preResolved_nextUrl = playerWindow.getProperty('luc_kodi.preResolved_nextUrl')
			...
			if self.mediatype == 'episode' and self.enable_playnext and int(control.playlist.size()) <= 1:
				try:
...
##################
			...
			# FALLBACK_SETRESOLVEDURL: si el player no arranca, forzar setResolvedUrl directamente
			try:
				from sys import argv as _argv
				from resources.lib.modules import control as _control
				_control.sleep(800)
				if not _control.condVisibility('Player.HasVideo') and url and url.startswith(('http','https')):
		...
		try:
			...
			for i in range(len(resolve_items)):
				try:
					...
					from sys import argv as _argv
					from resources.lib.modules import control as _control
					_item = _control.item(path=self.url)
					_control.resolve(int(_argv[1]), True, _item)
					# FASTPATH_SETRESOLVED_ITEM
					return self.url
				except: log_utils.error()
			try: self.progressDialog.close()
			except: pass
...
#				count += 1
	...
	def _ramcache_ttl_seconds(self):
		try:
			minutes = int(getSetting('sources.ramcache.ttl') or '5')
		except:
			minutes = 5
	...
	def _ramcache_set(self, cache_id, items):
		try:
			...
			homeWindow.setProperty('luc_kodi.ramcache.' + cache_id, jsdumps(items))
