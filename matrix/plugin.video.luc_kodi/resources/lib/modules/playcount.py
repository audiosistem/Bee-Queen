...
simklIndicators = (not traktIndicators) and simkl.getSimklIndicatorsInfo()
...
def getMovieOverlay(indicators, imdb):
	...
	try:
		if traktIndicators or simklIndicators:
			# Both return the same shape: list of IMDb IDs of watched movies.
...
def tvshowsUpdate(imdb, tvdb):
	...
	# def get(self, tvshowtitle, year, imdb, tmdb, tvdb, meta, season=None, episode=None, create_directory=True):
		...
		seasons_dict = {}
		for i in items:
			if i['season'] not in seasons_dict: seasons_dict[i['season']] = []
			seasons_dict[i['season']].append(i)

		countSeason = 0
		metaget.get_seasons('', imdb, seasons_dict.keys()) # Must be called to initialize the database.

		for key, value in iter(seasons_dict.items()):
			countEpisode = 0
			for i in value:
		...
		metaget.change_watched('tvshow', '', imdb_id=imdb, watched = 5 if countSeason == len(seasons_dict.keys()) else 4)
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
	containerRefresh()
