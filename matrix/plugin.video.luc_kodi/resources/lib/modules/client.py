			...
			if not ignore:
				if response.code in (301, 307, 308, 503, 403): # 403:Forbidden added 3/3/21 for cloudflare, fails on bad User-Agent
					...
					if flare and 'cloudflare' in str(response.info()).lower():
						...
						try:
							from jacksparrowscrapers.modules import cfscrape
...
class cfcookie:
		...
		try:
			...
			for line in lines:
				if len(line) > 0 and '=' in line:
					...
					decryptVal = int(literal_eval(str(decryptVal) + sections[0][-1] + str(line_val)))
	...
	def parseJSString(self, s):
		try:
			...
			val = int(literal_eval(s.replace('!+[]', '1').replace('!![]', '1').replace('[]', '0').replace('(', 'str(')[offset:]))
			return val
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
...
class sucuri:
	...
	def get(self, result):
		...
		try:
			...
			_ns = {'cookie': ''}
			exec(s, {'__builtins__': {}}, _ns)
			cookie = _ns.get('cookie', '')
