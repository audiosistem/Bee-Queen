# -*- coding: utf-8 -*-
"""
	plugin.video.luc_kodi — Torz (StremThru) Setup Wizard

	Flujo guiado para configurar Torz en MODO CUSTOM sin teclear el config
	blob a mano. Mismo patron que el wizard de Sootio/Meteor: ofrece dos
	...
	de Kodi (sin boton de pegar) tipico en Android TV / tablets:
	  ...
	  Metodo A) SERVIDOR LOCAL (recomendado, no requiere teclado)
	    ...
	    con clipboard nativo, pega el Manifest URL de StremThru y pulsa Save.
	...
	(_normalize_torz_token + _detect_debrid_from_config) y, si OK, guardan
	torz.config + torz.debrid.detected + activan provider.torz.

	El config de StremThru Torz lleva embebidas las credenciales del/los
	store(s) (RealDebrid, AllDebrid, TorBox, etc.), asi que tras el wizard NO
	hace falta autorizar ese debrid de nuevo dentro de luc_kodi.

	Invocacion desde settings.xml:
	  <setting type="action" label="..."
	           action="RunPlugin(plugin://plugin.video.luc_kodi/?action=tools_torzWizard)" />
"""
...
# Instancias publicas (mismo orden que el enum 'torz.url' en settings.xml).
# La pagina de configuracion cuelga de /stremio/torz/configure en cada una.
_INSTANCES = (
...
_LISTEN_PORT  = 48231   # puerto arbitrario poco usado (distinto del de Sootio)
...
def _configure_url():
	...
	try:
		idx = int(getSetting('torz.url') or '0')
	except Exception:
		idx = 0
	if idx < 0 or idx >= len(_INSTANCES):
		idx = 0
	return '%s/stremio/torz/configure' % _INSTANCES[idx]
...
def _detect_from_token(raw):
	"""Reutiliza normalizer + detector del scraper torz."""
	try:
		from resources.lib.jacksparrow.sourcesdir.torrents.torz import (
			_normalize_torz_token, _detect_debrid_from_config,
		)
		norm = _normalize_torz_token(raw)
...
def _save_token(raw_token, detected_label):
	...
	try:
		setSetting('torz.config', raw_token)
		setSetting('torz.debrid.detected', detected_label or 'not configured')
		setSetting('provider.torz', 'true')
		return True
	except Exception:
		return False
...
<title>StremThru Torz Setup</title>
<style>
...
<body>
<h1>StremThru Torz &mdash; paste your Manifest URL</h1>
<p>Open the StremThru Torz <code>configure</code> page, choose your Debrid
service(s) and paste your API key(s), then click <b>Install</b> and copy the
<b>Manifest URL</b> (it ends in <code>/manifest.json</code>). Paste it below and
press Save. You can close this page afterwards.</p>
<form method="post" action="/">
  <textarea name="token" autofocus placeholder="https://.../stremio/torz/.../manifest.json"></textarea>
...
<html><head><meta charset="utf-8"><title>Torz saved</title>
...
<html><head><meta charset="utf-8"><title>Torz error</title>
...
<p>Make sure you copy the full Manifest URL from the StremThru Torz configure page
(ending in /manifest.json).</p>
...
def _method_local_server():
	...
	while srv.is_alive() and srv.result is None:
		...
		pd.update(min(pct, 99), 'Torz Setup',
	...
	control.dialog.ok(
		'Torz Setup — done',
		'[COLOR ff00fa9a]Torz is configured and enabled (custom mode).[/COLOR]\n\n'
...
def _method_manual_paste():
	prior = getSetting('torz.config') or ''
	pasted = control.dialog.input(
		heading='Paste StremThru Torz Manifest URL',
		defaultt=prior,
		type=0,
	)
	if not pasted:
	...
	if not norm:
		control.dialog.ok(
			...
			'URL from the StremThru Torz configure page (ending in /manifest.json).',
		)
		return False

	if not _save_token(pasted, detected):
		control.dialog.ok('Torz Setup — error', 'Could not write settings.')
		return False
...
def run():
	...
	intro = (
		'Torz searches a crowdsourced torrent DB and resolves through YOUR '
		'Debrid (credentials travel inside the config token — no separate '
		'login in Kodi).\n\n'
...
def detect():
	"""Re-calcula el debrid detectado a partir del torz.config ya guardado,
	sin re-ejecutar el wizard. Util tras actualizar el addon o si el campo
	'Debrid detected' quedo desactualizado."""
	raw = getSetting('torz.config') or ''
	if not raw:
		control.dialog.ok(
			'Torz — re-detect',
			'No config token saved yet.\n\n'
			'Run the Torz Setup Wizard first, or paste a Manifest URL '
	...
	if not norm:
		...
		return
	setSetting('torz.debrid.detected', detected or 'not configured')
	# Asegura que el provider queda activo si hay token valido.
	if getSetting('provider.torz') != 'true':
		setSetting('provider.torz', 'true')
	control.dialog.ok(
		'Torz — re-detect',
		'[COLOR ff00fa9a]Done.[/COLOR]\n\n'
		'Debrid detected: [COLOR fffdb515]%s[/COLOR]\n\n'
		'Torz custom mode is active.' % (detected or 'not configured'),
	)
