...
def get_icon(image_name):
	skin_path = 'special://home/addons/%s/resources/skins/Default/media/%s.png'
	return skin_path % (addon_info('id'), image_name)

def get_addon_fanart():
	return get_property('magneto.addon_fanart') or default_addon_fanart

def build_url(url_params):
	return 'plugin://%s/?%s' % (addon_info('id'), urlencode(url_params))
...
def addon(addon_id=None):
	if not addon_id: addon_id = addon_info('id')
