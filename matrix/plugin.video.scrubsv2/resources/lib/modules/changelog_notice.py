...
containing resources/whatisnew.txt. The user MUST click OK to dismiss.
...
def _read_changelog():
    path = os.path.join(control.addonPath, 'resources', 'whatisnew.txt')
    try:
...
def show(force=False, auto_dismiss=True):
    ...
    heading = 'Scrubs v2 - What\'s New  (v%s)' % _current_version()
    try:
...
def service_check():
    ...
    try:
        if has_pending_notice():
            ...
            control.execute('RunPlugin(plugin://%s/?action=show_changelog)' % 'plugin.video.scrubsv2')
