...
def test_all():
    ...
    pd.create('Scrubs v2 - Scraper Tester', 'Testing providers...')
    ...
    try:
        xbmcgui.Dialog().textviewer('Scrubs v2 - Scraper Tester', body)
    except Exception:
        control.okDialog(body, 'Scrubs v2 - Scraper Tester')
    ...
    try:
        ...
        with open(out, 'w') as f:
            f.write('Scrubs v2 Scraper Tester - %s\n' % time.strftime('%Y-%m-%d %H:%M:%S'))
            for r in results:
                f.write('%s\t%s\t%s\t%s\t%dms\n' % (
...
def test_one():
    ...
    idx = xbmcgui.Dialog().select('Scrubs v2 - Select a Scraper to Test', names)
    ...
    try:
        pd.create('Scrubs v2', 'Testing %s...' % name)
    except Exception:
        pd = None
    ...
    msg = _format_line(r)
    try:
        xbmcgui.Dialog().textviewer('Scrubs v2 - Scraper Tester', msg)
    except Exception:
        control.okDialog(msg, 'Scrubs v2 - Scraper Tester')
