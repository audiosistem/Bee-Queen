...
def list_enabled_accounts():
  ...
  if common_vars.__config_voyo_Enabled__ == 'true':
    ...
    url = common_functions.get_url(action='list_categories', account='voyo.ro')
    common_vars.__logger__.debug('URL for plugin recursive call: ' + url)
  ...
  if params:
      if params['action'] == 'list_categories':
        ...
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__listCategories(common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__digionline_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')
        ...
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__listCategoryContent(params['id_category'], params['category_name'], params['category_page'], common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__voyo_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')


      elif params['action'] == 'list_tvshow_content':
        
        # voyo.ro
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__list_tvshow_content(params['tvshow_id'], common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__voyo_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')

      elif params['action'] == 'list_tvshow_season_episodes':
        
        # voyo.ro
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__list_tvshow_season_episodes(params['tvshow_id'], params['season_id'], common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__voyo_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')
        ...
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__listLiveTVChannels(params['id_category'], common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__voyo_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')
        ...
        if params['account'] == 'voyo.ro':
          if common_vars.__config_voyo_Enabled__ == 'true':
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Enabled')
            voyo_functions.voyo__playVideo(params['channel_id'], common_vars.__AddonID__, common_vars.__voyo_CookieJar__, common_vars.__voyo_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'voyo.ro\'  ==> Disabled')
      ...
      elif params['action'] == 'list_cachup_days':
        ...
        if params['account'] == 'digionline.ro':
          if common_vars.__config_digionline_Enabled__ == 'true':
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Enabled')
            if params['behaveas'] == "TV":
              digionline_functions.digionline__tv_listCatchupDays(common_vars.__AddonID__, common_vars.__digionline_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Disabled')

      elif params['action'] == 'list_cachup_channels':
        ...
        if params['account'] == 'digionline.ro':
          if common_vars.__config_digionline_Enabled__ == 'true':
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Enabled')
            if params['behaveas'] == "TV":
              digionline_functions.digionline__tv_listCatchupChannels(params['date'], common_vars.__AddonID__, common_vars.__digionline_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Disabled')

      elif params['action'] == 'list_cachup_events':
        ...
        if params['account'] == 'digionline.ro':
          if common_vars.__config_digionline_Enabled__ == 'true':
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Enabled')
            if params['behaveas'] == "TV":
              digionline_functions.digionline__tv_listCatchupEvents(params['date'], params['channel_id'], common_vars.__AddonID__, common_vars.__digionline_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Disabled')

      elif params['action'] == 'play_catchup_event':
        ...
        if params['account'] == 'digionline.ro':
          if common_vars.__config_digionline_Enabled__ == 'true':
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Enabled')
            if params['behaveas'] == "TV":
              digionline_functions.digionline__tv_playCatchupEvent(params['channel_id'], params['event_id'], common_vars.__AddonID__, common_vars.__digionline_Session__, MyAddon_DataDir)
          else:
            common_vars.__logger__.debug('\'digionline.ro\'  ==> Disabled')
