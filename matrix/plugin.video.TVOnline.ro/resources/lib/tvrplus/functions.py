...
def get_channels(NAME, COOKIEJAR, SESSION):
  ...
  _request_ = SESSION.get('https://www.tvrplus.ro', headers=MyHeaders, verify=False)
...
def play_video(CHANNEL_ENDPOINT, NAME, COOKIEJAR, SESSION, DATA_DIR):
  ...
  _request_ = SESSION.get(CHANNEL_ENDPOINT, headers=MyHeaders, verify=False)
...
def tvrplus__writeM3Ufile_record(M3U_FILE, CH_ID, CH_NAME, CH_LOGO, CH_NO, CH_URL, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  common_vars.__logger__.debug('    Channel ID => ' + CH_ID + ' Channel name: ' + CH_NAME + ' Channel logo: ' + CH_LOGO + ' Channel URL: ' + CH_URL)

  _line_ = "#EXTINF:0 tvg-id=\"tvrplus__" + CH_ID + "\" tvg-name=\"" + CH_NAME + "\" tvg-logo=\"" + CH_LOGO + "\" tvg-chno=\"" + CH_NO + "\" group-title=\"TVR Plus\"," + CH_NAME
    
  _url_ = common_functions.get_url(action='play', account='tvrplus.ro', channel_endpoint=CH_URL)
...
def PVRIPTVSimpleClientIntegration_update_m3u_file(M3U_FILE, START_NUMBER, NAME, COOKIEJAR, SESSION, DATA_DIR):
  ...
  for channel in channels:
    ...
    if common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_map__[common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__] == "None":
      ...
      tvrplus__writeM3Ufile_record(_data_file_, str(channel['id']), channel['name'], channel['logo'], str(_CHNO_), channel['url'], NAME, COOKIEJAR, SESSION)

    _search_string_ = "tvrplus__" + str(channel['id'])
