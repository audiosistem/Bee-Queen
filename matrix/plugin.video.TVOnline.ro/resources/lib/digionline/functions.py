...
def digionline__tv_init_stateData(NAME, DATA_DIR):
  ...
  __metadata__ = {}
  __metadata__['version'] = "2"

  __token__ = {}
  __token__['token_type'] = ""
  __token__['access_token'] = ""
  __token__['refresh_token'] = ""
...
def digionline__tv_API_getToken(NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  __URL__ = 'https://apitv.digionline.ro/v1/oauth2/token'
  ...
  MyPayloadData = {
      "grant_type": "password",
      "client_id": str(base64.b64decode(common_vars.__digionline_TV_API_client_i__), "utf-8"),
      "client_secret": str(base64.b64decode(common_vars.__digionline_TV_API_client_s__), "utf-8"),
      "device_id": __rsd__['state_data']['data']['deviceID'],
      "username": common_vars.__config_digionline_Username__,
...
def digionline__tv_API_refreshToken(NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  MyPayloadData = {
      ...
      "client_id": str(base64.b64decode(common_vars.__digionline_TV_API_client_i__), "utf-8"),
      "client_secret": str(base64.b64decode(common_vars.__digionline_TV_API_client_s__), "utf-8"),
      "refresh_token": __rsd__['state_data']['data']['token']['refresh_token'],
      "device_id": __rsd__['state_data']['data']['deviceID'],
      "model": common_vars.__config_digionline_TVDeviceModel__,
      "platform": common_vars.__config_digionline_TVPlatform__,
      "processor": common_vars.__config_digionline_TVAndroidVersion__
...
def digionline__tv_getCategoriesChannels(NAME, SESSION, DATA_DIR):
  ...
  __URL__ = 'https://apitv.digionline.ro/v1/api/channels_categories'
...
def digionline__tv_getChannelDetails(CHANNEL_ID, NAME, SESSION, DATA_DIR):
  ...
  __URL__ = 'https://apitv.digionline.ro/v1/api/channel'
  ...
  MyHeaders = {
    'User-Agent': common_vars.__digionline_API_userAgent__,
    'Accept': 'application/json, text/plain, */*',
    'Authorization': __rsd__['state_data']['data']['token']['token_type'] + ' ' + str(__rsd__['state_data']['data']['token']['access_token'])
  }
...
def digionline__tv_getEPG(DATE, NAME, SESSION, DATA_DIR):
  ...
  __URL__ = 'https://apitv.digionline.ro/v1/api/epgs-full/byDate/' + str(DATE)
...
def digionline__tv_getCatchUpByDate(DATE, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  __URL__ = 'https://apitv.digionline.ro/v1/api/catchup/byDate/' + str(DATE)
...
def digionline__tv_getCatchUpEventDetails(CHANNEL_ID, EVENT_ID, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  __URL__ = 'https://apitv.digionline.ro/v1/api/catchup/event'
...
def digionline__tv_checkToken(NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  if __rsd__['exit_status'] != 0 or int(__rsd__['state_data']['metadata']['version']) < 2 or __rsd__['state_data']['data']['deviceID'] == "":
    digionline__tv_init_stateData(NAME, DATA_DIR)
    __rsd__ = digionline__tv_read_stateData(NAME, DATA_DIR)

    __new_token__ = digionline__tv_API_getToken(NAME, SESSION, DATA_DIR)
    common_vars.__logger__.debug('__new_token__ = ' + str(__new_token__))

    if 'meta' in __new_token__:
      xbmcgui.Dialog().ok('[digionline.ro] => Authentication error (code ' + str(__new_token__['meta']['error']['code']) + ' )', str(__new_token__['meta']['error']['message']))
      common_vars.__logger__.debug('[digionline.ro] => Authentication error code: ' + str(__new_token__['meta']['error']['code']))
      common_vars.__logger__.debug('[digionline.ro] => Authentication error message: ' + str(__new_token__['meta']['error']['message']))
      common_vars.__logger__.debug('Exit function')
      return

    else:
      __rsd__ = digionline__tv_read_stateData(NAME, DATA_DIR)
      __rsd__['state_data']['data']['token']['token_type'] = __new_token__['token_type']
      __rsd__['state_data']['data']['token']['access_token'] = __new_token__['access_token']
      __rsd__['state_data']['data']['token']['refresh_token'] = __new_token__['refresh_token']
  ...
  if __rsd__['state_data']['data']['token']['token_type'] == "" or __rsd__['state_data']['data']['token']['access_token'] == "" or __rsd__['state_data']['data']['token']['refresh_token'] == "":
  ...
  if 'meta' in __check_token__:
    if 'error' in __check_token__['meta']:
      if 'expired' in __check_token__ and __check_token__['expired']:
        # Access token is expired.
        common_vars.__logger__.debug('Access token is expired')
        __refresh_token__ = digionline__tv_API_refreshToken(NAME, SESSION, DATA_DIR)

        if 'meta' in __refresh_token__:
          if 'error' in __refresh_token__['meta']:
            if 'logout' in __refresh_token__ and __refresh_token__['logout']:
              # Refresh token is revoked. Get new tokens.
              common_vars.__logger__.debug('Refresh token is revoked.')
              __new_token__ = digionline__tv_API_getToken(NAME, SESSION, DATA_DIR)
              common_vars.__logger__.debug('__new_token__ = ' + str(__new_token__))
              ...
              if 'meta' in __new_token__:
                ...
                common_vars.__logger__.debug('New token was obtained.')
                __rsd__ = digionline__tv_read_stateData(NAME, DATA_DIR)
                __rsd__['state_data']['data']['token']['token_type'] = __new_token__['token_type']
                __rsd__['state_data']['data']['token']['access_token'] = __new_token__['access_token']
                __rsd__['state_data']['data']['token']['refresh_token'] = __new_token__['refresh_token']
          ...
          # The refresh token is not revoked but there was an error reported when refreshing the access token
          else:
            xbmcgui.Dialog().ok('[digionline.ro] => Error (code ' + str(__check_token__['meta']['error']['code']) + ' )', str(__check_token__['meta']['error']['message']))
            common_vars.__logger__.debug('[digionline.ro] => Error code: ' + str(__check_token__['meta']['error']['code']))
            common_vars.__logger__.debug('[digionline.ro] => Error message: ' + str(__check_token__['meta']['error']['message']))

        # Refresh access token was successful
        else:
          common_vars.__logger__.debug('Access token was refreshed.')
          __rsd__ = digionline__tv_read_stateData(NAME, DATA_DIR)
          __rsd__['state_data']['data']['token']['token_type'] = __refresh_token__['token_type']
          __rsd__['state_data']['data']['token']['access_token'] = __refresh_token__['access_token']
          __rsd__['state_data']['data']['token']['refresh_token'] = __refresh_token__['refresh_token']
          digionline__tv_write_stateData(__rsd__['state_data'], NAME, DATA_DIR)

      # The access token is not expired but there was an error reported when checking it
      else:
        xbmcgui.Dialog().ok('[digionline.ro] => Error (code ' + str(__check_token__['meta']['error']['code']) + ' )', str(__check_token__['meta']['error']['message']))   
        common_vars.__logger__.debug('[digionline.ro] => Error code: ' + str(__check_token__['meta']['error']['code']))
        common_vars.__logger__.debug('[digionline.ro] => Error message: ' + str(__check_token__['meta']['error']['message']))
  ...
  # The access token is not expired
    ...
    common_vars.__logger__.debug('Access token is not expired.')
...
def digionline__listCategories(BEHAVE_AS, NAME, SESSION, DATA_DIR):
  ...
  if BEHAVE_AS == "TV":
    ...
    digionline__tv_checkToken(NAME, SESSION, DATA_DIR)

    # Get video categories
    categories_channels = digionline__tv_getCategoriesChannels(NAME, SESSION, DATA_DIR)
    common_vars.__logger__.debug('Received data = ' + str(categories_channels))

    for category_data in categories_channels['data']['categories_list']:
    ...
    ##### Catchup "category"
    # Create a list item with a text label and a thumbnail image.
    list_item = xbmcgui.ListItem(label='=== Catch-up ===')

    # Set additional info for the list item.
    # For available properties see https://codedocs.xyz/xbmc/xbmc/group__python__xbmcgui__listitem.html#ga0b71166869bda87ad744942888fb5f14
    # 'mediatype' is needed for a skin to display info for this ListItem correctly.
    list_item.setInfo('video', {'title': '=== Catch-up ===',
                                'mediatype': 'video'})

    # Create a URL for a plugin recursive call.
    # Example: plugin://plugin.video.example/?action=listing&category=filme
    url = common_functions.get_url(account='digionline.ro', behaveas=common_vars.__behave_map__[common_vars.__config_digionline_BehaveAs__], action='list_cachup_days')
    common_vars.__logger__.debug('URL for plugin recursive call: ' + url)
...
def digionline__tv_listChannels(ID_CATEGORY, CATEGORY_NAME, NAME, SESSION, DATA_DIR):
  ...
  for channel_data in categories_channels['data']['channels_list']:
  ...
  if BEHAVE_AS == "Phone":
    ...
    if _stream_details_['stream']['f'] == "hls":
      ...
      if is_helper.check_inputstream():
        play_item = xbmcgui.ListItem(path=_stream_details_['stream']['cast']['drm'] + '|' + _headers_)
    ...
    if _stream_details_['stream']['f'] == "dash":
      ...
      _lic_headers_host_ = re.findall('//(.+?)/', _stream_details_['stream']['cast']['drm'], re.IGNORECASE)[0]
...
#      _lic_headers_ = _lic_headers_ + '&User-Agent=user-agent'
      ...
      if is_helper.check_inputstream():
        play_item = xbmcgui.ListItem(path=_stream_details_['stream']['cast']['drm'])
        play_item.setProperty('inputstream', 'inputstream.adaptive')
        play_item.setProperty('inputstream.adaptive.manifest_headers', _headers_)
        play_item.setProperty('inputstream.adaptive.stream_headers', _headers_)
  ...
  if BEHAVE_AS == "TV":
    ...
    if _channel_details_['meta']:
      if _channel_details_['meta']['error']:
        xbmcgui.Dialog().ok('[digionline.ro] => Error code: ' + str(_channel_details_['meta']['error']['code']), str(_channel_details_['meta']['error']['message']))
        common_vars.__logger__.debug('[digionline.ro] => Error code: ' + str(_channel_details_['meta']['error']['code']))
        common_vars.__logger__.debug('[digionline.ro] => Error message: ' + str(_channel_details_['meta']['error']['message']))
        common_vars.__logger__.debug('Exit function')
        return
...
def digionline__phone_writeM3Ufile_record(M3U_FILE, CH_ID, CH_NAME, CH_TITLE, CH_LOGO, CH_NO, CH_GROUP_NAME, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  
  common_vars.__logger__.debug('Channel ID => ' + CH_ID)
  common_vars.__logger__.debug('Channel name: ' + CH_NAME)
  common_vars.__logger__.debug('Channel title: ' + CH_TITLE)
  common_vars.__logger__.debug('Channel logo: ' + CH_LOGO)
  common_vars.__logger__.debug('Channel number: ' + CH_NO)
  common_vars.__logger__.debug('Channel group name: ' + CH_GROUP_NAME)

  _line_ = "#EXTINF:0 tvg-id=\"digionline__" + CH_ID + "\" tvg-name=\"" + CH_TITLE + "\" tvg-logo=\"" + CH_LOGO + "\" tvg-chno=\"" + CH_NO + "\" group-title=\"" + CH_GROUP_NAME + "\"," + CH_TITLE

  _url_ = common_functions.get_url(account='digionline.ro', behaveas=common_vars.__behave_map__[common_vars.__config_digionline_BehaveAs__], action='play', channel_id=CH_ID)
    ...
    for cat_chan in category['channels_list']:
      for channel in _channels_:
        if cat_chan == channel['id_channel']:
          digionline__phone_writeM3Ufile_record(_data_file_, str(channel['id_channel']), str(channel['channel_name']), str(channel['channel_desc']), str(channel['media_channel']['channel_logo_url']), str(_CHNO_), category['category_desc'], NAME, SESSION, DATA_DIR)
...
def digionline__tv_writeM3Ufile_record(M3U_FILE, CH_ID, CH_NAME, CH_LOGO, CH_NO, CH_GROUP_NAME, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  common_vars.__logger__.debug('    Channel ID => ' + CH_ID + ' Channel name: ' + CH_NAME + ' Channel logo: ' + CH_LOGO)

  _line_ = "#EXTINF:0 tvg-id=\"digionline__" + CH_ID + "\" tvg-name=\"" + CH_NAME + "\" tvg-logo=\"" + CH_LOGO + "\" tvg-chno=\"" + CH_NO + "\" group-title=\"" + CH_GROUP_NAME + "\"," + CH_NAME
...
def digionline__tv_updateM3Ufile(M3U_FILE, START_NUMBER, NAME, SESSION, DATA_DIR):
  ...
  # Get the list of categories and channels
  _categories_channels_ = digionline__tv_getCategoriesChannels(NAME, SESSION, DATA_DIR)
  common_vars.__logger__.debug('Received data = ' + str(_categories_channels_))

  for _category_ in _categories_channels_['data']['categories_list']:
    ...
    for _channel_ in _categories_channels_['data']['channels_list']:
      if "channel_categories" in _channel_:
        if _category_['id_category'] in _channel_['channel_categories']:
          ...
          if common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_map__[common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__] == "None":
            ...
            digionline__tv_writeM3Ufile_record(_data_file_, str(_channel_['id']), str(_channel_['name']), str(_channel_['media_channel']['logo']), str(_CHNO_), _category_['category_desc'], NAME, SESSION, DATA_DIR)

          _search_string_ = "digionline__" + str(_channel_['id'])
          ...
          if common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_map__[common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__] == "Blacklist":
            ...
            common_vars.__logger__.debug('Blacklist list: ' + str(_filter_lists_data_['blacklist']))
          ...
          if common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_map__[common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__] == "Whitelist":
            ...
            common_vars.__logger__.debug('Whitelist list: ' + str(_filter_lists_data_['whitelist']))
...
#  common_vars.__logger__.debug('_epg_data_ = ' + str(_epg_data_))
  ...
  for channel in _epg_data_:
    ...
    for _program_data_ in channel['channel_epg']:
      ...
      _data_file_.write(_line_ + "\n")
      common_vars.__logger__.debug('_program_data_[\'n\'] => ' + str(_program_data_['n']))
...
def digionline__tv_listCatchupDays(NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  __catchup_data__ = digionline__tv_getCatchUpByDate(_today_, NAME, SESSION, DATA_DIR)

  for x in range(0, __catchup_data__['meta']['availabledays']):

    __date__ = datetime.date(datetime.today()) - timedelta(days=x)
    common_vars.__logger__.debug('__date__: ' + str(__date__))

    # Create a list item with a text label and a thumbnail image.
    list_item = xbmcgui.ListItem(label=str(__date__))

    # Set additional info for the list item.
    # For available properties see https://codedocs.xyz/xbmc/xbmc/group__python__xbmcgui__listitem.html#ga0b71166869bda87ad744942888fb5f14
    # 'mediatype' is needed for a skin to display info for this ListItem correctly.
    list_item.setInfo('video', {'title': str(__date__),
                                'mediatype': 'video'})

    # Create a URL for a plugin recursive call.
    # Example: plugin://plugin.video.example/?action=listing&category=filme
    url = common_functions.get_url(account='digionline.ro', behaveas=common_vars.__behave_map__[common_vars.__config_digionline_BehaveAs__], action='list_cachup_channels', date=__date__)
    common_vars.__logger__.debug('URL for plugin recursive call: ' + url)
  ...
  # Add a sort method for the virtual folder items (alphabetically, ignore articles)
  # See: https://romanvm.github.io/Kodistubs/_autosummary/xbmcplugin.html
  xbmcplugin.addSortMethod(int(common_vars.__handle__), xbmcplugin.SORT_METHOD_NONE)
...
def digionline__tv_listCatchupChannels(DATE, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  # Get the list of categories and channels
  __channel_data__ = digionline__tv_getCategoriesChannels(NAME, SESSION, DATA_DIR)

  # Get the list of catchup channels and events
  __catchup_data__ = digionline__tv_getCatchUpByDate(DATE, NAME, SESSION, DATA_DIR)

  for catchup_channel in __catchup_data__['data']['channels']:
    common_vars.__logger__.debug('catchup_channel: ' + catchup_channel)
    common_vars.__logger__.debug('catchup_channel channel ID: ' + __catchup_data__['data']['channels'][catchup_channel]['id'])
    common_vars.__logger__.debug('catchup_channel channel name: ' + __catchup_data__['data']['channels'][catchup_channel]['name'])

    # Create a list item with a text label and a thumbnail image.
    list_item = xbmcgui.ListItem(label=__catchup_data__['data']['channels'][catchup_channel]['name'])

    # Set additional info for the list item.
    # For available properties see https://codedocs.xyz/xbmc/xbmc/group__python__xbmcgui__listitem.html#ga0b71166869bda87ad744942888fb5f14
    # 'mediatype' is needed for a skin to display info for this ListItem correctly.
    list_item.setInfo('video', {'title': __catchup_data__['data']['channels'][catchup_channel]['name'],
                                'mediatype': 'video'})

    # Set graphics (thumbnail, fanart, banner, poster, landscape etc.) for the list item.
    #list_item.setArt({'thumb': __channel_logo__})

    # Create a URL for a plugin recursive call.
    # Example: plugin://plugin.video.example/?action=listing&category=filme
    url = common_functions.get_url(account='digionline.ro', behaveas=common_vars.__behave_map__[common_vars.__config_digionline_BehaveAs__], action='list_cachup_events', date=DATE, channel_id=__catchup_data__['data']['channels'][catchup_channel]['id'])
    common_vars.__logger__.debug('URL for plugin recursive call: ' + url)
...
def digionline__tv_listCatchupEvents(DATE, CHANNEL_ID, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  for _event_data_ in __catchup_data__['data']['channels'][CHANNEL_ID]['epg']:
    _start_date_time_object_ = datetime.utcfromtimestamp(int(_event_data_['s']/1000))
    common_vars.__logger__.debug('Event ID: ' + str(_event_data_['i']) + '  |  Event Time: ' + str(_start_date_time_object_.strftime("%H:%M")) + '  |  Event: ' + _event_data_['n'])

    # Create a list item with a text label and a thumbnail image.
    list_item = xbmcgui.ListItem(label=str(_start_date_time_object_.strftime("%H:%M")))

    # Set additional info for the list item.
    # For available properties see https://codedocs.xyz/xbmc/xbmc/group__python__xbmcgui__listitem.html#ga0b71166869bda87ad744942888fb5f14
    # 'mediatype' is needed for skin to display info for this ListItem correctly.

    list_item.setInfo('video', {'title': '[ ' + str(_start_date_time_object_.strftime("%H:%M")) + ' ] ' + _event_data_['n'],
                                'genre': 'Catch-up event',
                                'mediatype': 'video'})
    ...
    # Create a URL for a plugin recursive call.
    # Example: plugin://plugin.video.example/?action=play&channel_endpoint=/filme/tnt&channel_metadata=...
    url = common_functions.get_url(account='digionline.ro', behaveas=common_vars.__behave_map__[common_vars.__config_digionline_BehaveAs__], action='play_catchup_event', channel_id=CHANNEL_ID, event_id=_event_data_['i'])
    common_vars.__logger__.debug('URL for plugin recursive call: ' + url)
...
def digionline__tv_playCatchupEvent(CHANNEL_ID, EVENT_ID, NAME, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  _channel_details_ = digionline__tv_getCatchUpEventDetails(CHANNEL_ID, EVENT_ID, NAME, SESSION, DATA_DIR)
  common_vars.__logger__.debug('Received data: ' + str(_channel_details_))
  ...
  if _channel_details_['data']['catchup_url'] != "" and _channel_details_['data']['catchup_proxy'] != "":
...
#    _lic_headers_ = _lic_headers_ + '&verifypeer=false'
    ...
    is_helper = inputstreamhelper.Helper('mpd', drm='com.widevine.alpha')
    if is_helper.check_inputstream():
      play_item = xbmcgui.ListItem(path=_channel_details_['data']['catchup_url'])
      play_item.setProperty('inputstream', 'inputstream.adaptive')
      play_item.setProperty('inputstream.adaptive.manifest_headers', _headers_)
      play_item.setProperty('inputstream.adaptive.stream_headers', _headers_)
      play_item.setProperty('inputstream.adaptive.license_type', 'com.widevine.alpha')
      play_item.setProperty('inputstream.adaptive.license_key', _channel_details_['data']['catchup_proxy'] + '|' + _lic_headers_ + '|R{SSM}|')
      play_item.setMimeType('application/dash+xml')
