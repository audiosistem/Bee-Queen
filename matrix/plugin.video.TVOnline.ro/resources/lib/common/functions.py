...
def read_AddonSettings(__MyAddon__, NAME):
  ...
  common_vars.__config_voyo_DeviceName__ = __MyAddon__.getSetting('voyo_DeviceName')
  ...
  common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__ = __MyAddon__.getSetting('PVRIPTVSimpleClientIntegration_FilterType')
  common_vars.__logger__.debug('[ Addon settings ] PVRIPTVSimpleClientIntegration_FilterType = ' + str(common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__))
...
def readChannelFilterLists(NAME, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  _filter_lists_file_ = os.path.join(DATA_DIR, common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_Lists_FileName__)
  if not os.path.exists(_filter_lists_file_):
    _fld_ = {}
    _fld_['blacklist'] = []
    _fld_['whitelist'] = []

    _file_ = open(_filter_lists_file_, 'w')
    json.dump(_fld_, _file_)
    _file_.close()
  else:
    _file_ = open(_filter_lists_file_, 'r')
    try:
      _fld_ = json.load(_file_)
    except:
      common_vars.__logger__('File \'' + _filter_lists_file_ + '\' does not contain a valid json format.')
      common_vars.__logger__('Returning empty lists.')
      _fld_ = {}
      _fld_['blacklist'] = []
      _fld_['whitelist'] = []
