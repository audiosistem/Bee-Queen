...
def play_video(CHANNEL_ENDPOINT, NAME, COOKIEJAR, SESSION, DATA_DIR):
  ...
  if _premium_content_:
    ...
    _raw_stream_manifest_url_ = re.findall('<div class="container-video position-relative" id="video_thumb">(.+?)</div>', _request_.content.decode(), re.IGNORECASE|re.DOTALL)[0]
    common_vars.__logger__.debug('Found _raw_stream_manifest_url_ = ' + str(_raw_stream_manifest_url_))
    
    _stream_manifest_url_ = urllib.parse.unquote(re.findall('playbackURL=(.+?)&viewerID', _raw_stream_manifest_url_, re.IGNORECASE|re.DOTALL)[0])
...
def primaplay__writeM3Ufile_record(M3U_FILE, CH_ID, CH_NAME, CH_LOGO, CH_NO, CH_URL, NAME, COOKIEJAR, SESSION):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')

  common_vars.__logger__.debug('    Channel ID => ' + CH_ID + ' Channel name: ' + CH_NAME + ' Channel logo: ' + CH_LOGO + ' Channel URL: ' + CH_URL)
  
  _line_ = "#EXTINF:0 tvg-id=\"primaplay__" + CH_ID + "\" tvg-name=\"" + CH_NAME + "\" tvg-logo=\"" + CH_LOGO + "\" tvg-chno=\"" + CH_NO + "\" group-title=\"Prima Play\"," + CH_NAME
    
  _url_ = common_functions.get_url(action='play', account='primaplay.ro', channel_endpoint=CH_URL)
...
def PVRIPTVSimpleClientIntegration_update_m3u_file(M3U_FILE, START_NUMBER, NAME, COOKIEJAR, SESSION, DATA_DIR):
  common_vars.__logger__ = logging.getLogger(NAME)
  common_vars.__logger__.debug('Enter function')
  ...
  for channel in channels:
    ...
    if common_vars.__PVRIPTVSimpleClientIntegration__ChannelFilter_map__[common_vars.__config_PVRIPTVSimpleClientIntegration_FilterType__] == "None":
      ...
      primaplay__writeM3Ufile_record(_data_file_, str(channel['id']), channel['name'], channel['logo'], str(_CHNO_), channel['url'], NAME, COOKIEJAR, SESSION)

    _search_string_ = "primaplay__" + str(channel['id'])
