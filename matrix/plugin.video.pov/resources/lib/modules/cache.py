...
def check_databases():
	...
	watched_schema = [
		('CREATE TABLE IF NOT EXISTS watched_status '
		 ...
		 'last_played TEXT, title TEXT, UNIQUE (db_type, media_id, season, episode))'),
		('CREATE TABLE IF NOT EXISTS progress '
		 '(db_type TEXT, media_id TEXT, season INTEGER, episode INTEGER, '
		 'resume_point TEXT, curr_time TEXT, last_played TEXT, resume_id INTEGER, '
		 'title TEXT, UNIQUE (db_type, media_id, season, episode))'),
		('CREATE INDEX IF NOT EXISTS pov_ws_in_progress_episodes '
		 'ON watched_status (db_type, media_id, season DESC, episode DESC)')
	]
	db_mapping = {
		maincache_db: [
		...
		metacache_db: [
			...
			'CREATE INDEX IF NOT EXISTS pov_select_id_media ON metadata (tmdb_id, db_type)'
		],
		favorites_db: [
			'CREATE TABLE IF NOT EXISTS favorites (db_type TEXT, tmdb_id TEXT, title TEXT, UNIQUE (db_type, tmdb_id))',
			'CREATE TABLE IF NOT EXISTS dropped (db_type TEXT, tmdb_id TEXT, title TEXT, UNIQUE (db_type, tmdb_id))'
		],
		views_db: [
		...
		external_db: [
			('CREATE TABLE IF NOT EXISTS results_data '
			 '(provider TEXT, db_type TEXT, tmdb_id TEXT, title TEXT, year INTEGER, '
			 'season TEXT, episode TEXT, results TEXT, expires INTEGER, '
			 'UNIQUE (provider, db_type, tmdb_id, title, year, season, episode))')
		],
		watched_db:   watched_schema,
		trakt_db:     watched_schema,
		mdbl_db:      watched_schema
	}
	for db_path, queries in db_mapping.items():
		dbcon = database_connect(db_path)
		for query in queries: dbcon.execute(query)
		dbcon.close()
