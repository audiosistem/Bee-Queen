...
class source:
	...
	def search(self, imdb, season, episode):
		...
		try:
			base_url = self.resolve_aio_instance(get_setting('aio.instance', '0'))
	...
	def resolve_aio_instance(self, aio_instance_value):
		setting_id = (
			'aio.ku_url',
			'aio.custom_url',
			'aio.viren_url',
...
highly customizable stream provider.  When enabled AIOStreams supersedes
