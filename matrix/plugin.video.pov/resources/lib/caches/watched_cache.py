...
def dt_formats(watched_indicators):
	if watched_indicators == 1: return '1970-01-01T00:00:00.000Z'
	if watched_indicators == 2: return '1970-01-01T00:00:00Z'
	return '1970-01-01 00:00:00'

def get_last_played_value(watched_indicators):
	if watched_indicators == 1: return datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%S.000Z')
	if watched_indicators == 2: return datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
...
def set_resumetime(resumetime, progress, infotag):
	try: resumetime, progress = float(resumetime), float(progress)
	except: resumetime, progress = float(0), float(0)
	if not resumetime and not progress: return
	duration = infotag.getDuration()
	if resumetime: return infotag.setResumePoint(resumetime, duration)
	if progress: return infotag.setResumePoint(progress * duration / 100, duration)
...
def get_bookmarks(watched_indicators, mediatype):
	try:
		dbcon = _database_connect(watched_indicators)
		dbcur = set_PRAGMAS(dbcon)
...
#		erase_bookmark(mediatype, tmdb_id, season, episode)
		...
		function('set_progress', mediatype, tmdb_id, resume_point, season, episode, refresh=True)
	except: pass
...
def erase_bookmark(mediatype, tmdb_id, season='', episode='', refresh='false'):
	try:
		...
		if watched_indicators not in (1, 2): return
		function = mdbl_progress if watched_indicators == 2 else trakt_progress
		function('clear_progress', mediatype, tmdb_id, 0, season, episode, resume_id)
	except: pass
...
def mark_as_watched_unwatched_movie(params):
	...
	if watched_indicators == 1:
		if not from_playback or trakt_official_status(mediatype) is not False:
			trakt_watched_unwatched(action, 'movies', tmdb_id)
		clear_trakt_collection_watchlist_data('watchlist', mediatype)
	elif watched_indicators == 2:
		mdbl_watched_unwatched(action, 'movies', tmdb_id)
...
def mark_as_watched_unwatched_episode(params):
	...
	if season == 0: return kodi_utils.notify_error()
	...
	if watched_indicators == 1:
		if not from_playback or trakt_official_status(mediatype) is not False:
			trakt_watched_unwatched(action, mediatype, tmdb_id, tvdb_id, season, episode)
		clear_trakt_collection_watchlist_data('watchlist', 'tvshow')
	elif watched_indicators == 2:
		mdbl_watched_unwatched(action, mediatype, tmdb_id, tvdb_id, season, episode)
		clear_mdbl_collection_watchlist_data('watchlist')
...
def mark_as_watched_unwatched_tvshow(params):
	...
	kodi_utils.progressDialogBG.create(plswait_str, '')
	try:
		...
		for count, item in enumerate(ep_data, 1):
			...
			kodi_utils.progressDialogBG.update(int(float(count)/float(total)*100), plswait_str, display)
