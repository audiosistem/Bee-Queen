...
def _follow_stream_url(url, timeout=10):
	"""Follow redirects on resolved stream URL, bypass ad/interstitial pages."""
	stream_headers = {
	...
	try:
		resp = requests.head(url, headers=stream_headers, timeout=timeout, allow_redirects=True)
		final_url = resp.url
		content_type = resp.headers.get('Content-Type', '')
		if 'text/html' in content_type:
			log(f"Detected HTML redirect (possible ad page), trying to extract stream URL")
			resp_get = requests.get(url, headers=stream_headers, timeout=timeout, allow_redirects=True)
		...
		log(f"Failed to follow URL, using original: {e}")
...
def resolve_link(link):
	try:
		if not "vavoo" in link:
			...
			_data = {"language": "de", "region": "AT", "url": link, "clientVersion": "3.1.4"}
			url = "https://vavoo.to/mediahubmx-resolve.json"
			result = requests.post(url, json=_data, headers=_headers).json()
			streamurl = result[0]["url"]
			# Follow redirects to bypass ad/interstitial pages
			final_url, stream_headers = _follow_stream_url(streamurl)
			# Pass auth signature and referer to CDN
			if sig: stream_headers["mediahubmx-signature"] = sig
			stream_headers["Referer"] = "https://vavoo.to/"
			stream_headers["Origin"] = "https://vavoo.to"
			log(f"function resolve_link resolved: {final_url}")
			headers_str = "&".join([f"{k}={url_quote(v)}" for k, v in stream_headers.items()])
			return final_url, headers_str
		else:
...
def livePlay(name, type=None, group=None):
	...
	if headers:
		if inputstream == "inputstream.adaptive":
			...
			url += f"|{headers}"
			o.setProperty("inputstream.ffmpegdirect.headers", headers)
	o.setPath(url)
