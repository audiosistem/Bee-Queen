...
_KODI_TEMP = xbmcvfs.translatePath('special://temp/dltv/')
...
SEED_BASEURL = _seed_setting if _seed_setting else 'https://dlhd.pk/'
...
_seg_semaphore = threading.Semaphore(6)  # mărit la 6 pentru fluidizare zapping
    ...
    if ip is None:
        ...
        if ip:
            ...
            if ip:
                ...
                return _original_getaddrinfo(host, port, family, type, proto, flags)
    try:
        # Folosim apelul nativ pe IP-ul rezolvat pentru a genera structura corectă de socket cerută de Windows
...
def _extract_creds_with_js(page_text, page_url, base_url, sess):
    ...
    for sm in re.finditer(r'<script[^>]+src=["\']([^"\']+)["\']', page_text):
        ...
        try:
            ...
            rjs = sess.get(src, headers={'User-Agent': _AUTH_UA, 'Referer': page_url}, timeout=20)
...
def _fetch_auth_credentials(channel_id, max_attempts=3):
    ...
    # so a prior visit to dlstreams.top unblocks adffdafdsafds.sbs in the same session.
    stream_referer = abs_url(f'stream/stream-{channel_id}.php')
    try:
        _get_session().get(stream_referer, headers={'User-Agent': UA}, timeout=20)
    except Exception:
        pass
...
def _discover_server_urls():
    ...
    try:
        ...
        if chevy_proxy and not chevy_lookup:
            ...
            try:
                rh = _get_session().get(f'{candidate}/health', headers={'User-Agent': UA}, timeout=20)
    ...
    try:
        ...
        for mm in _AP_M3U8_RX.finditer(html):
            ...
            if 'blogspot.com' in candidate or 'r-strm.' in candidate:
                continue
            try:
                rp = sess.get(candidate, headers={'User-Agent': UA}, timeout=20)
                        ...
                        try:
                            ...
                            if _ll_r.status_code == 200:
                                for _ll_mi in _AP_M3U8_RX.finditer(_ll_r.text):
                                    ...
                                    try:
                                        _ll_rp = sess.get(_ll_cand, headers={'User-Agent': UA}, timeout=20)
                    ...
                    for srv in srv_list:
                        try:
                            sl_r = sess.get(f'{srv}/server_lookup?channel_id={vkey}',
                                            headers={'User-Agent': UA, 'Referer': iframe_url}, timeout=20)
