...
import inspect
from gearsscrapers.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.gearsscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ script.module.gearsscrapers ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from gearsscrapers.modules.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.gearsscrapers ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		from gearsscrapers.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from gearsscrapers.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from gearsscrapers.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'script.kodi.loguploader: 1.0' #'gearsscrapers %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			select = selectDialog([i[0] for i in list], 'gearsscrapers log file uploaded to:')
			if 'Copy url To Clipboard' in list[select][0]:
				from gearsscrapers.modules.source_utils import copy2clip
