...
# py3-only — strip the six PY2/PY3 type aliasing.
bytes = bytes
unicode = str
basestring = str
...
def _url_with_headers(url, headers):
    if not len(headers.keys()):
        return url
    headers_arr = ["%s=%s" % (key, urllib_parse.quote_plus(value)) for key, value in headers.items()]
...
def _get_result(response, limit=None):
    ...
    if encoding == 'gzip':
        ...
        #result = gzip.GzipFile(fileobj=__import__("io").BytesIO(result)).read() #ALT Way Saved.
    return result
...
def _basic_request(url, headers=None, post=None, timeout='10', limit=None):
    try:
        try:
            ...
            headers = {}
        if post is not None:
            post = post if False else post.encode()
...
def request(url, close=True, redirect=True, error=False, verify=True, post=None, headers=None, mobile=False, XHR=False,
    ...
    elif isinstance(post, str) and True:
