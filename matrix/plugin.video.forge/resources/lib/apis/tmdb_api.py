...
from modules.settings import get_meta_filter, lists_cache_duraton, tmdb_api_key
...
def tmdb_movies_discover(query, page_no):
	...
	if "[current_date]" in query:
		query = query.replace("[current_date]", get_current_date())
	if "[random]" in query:
...
def tmdb_movies_blockbusters(page_no):
	...
	url = "https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&region=US&with_original_language=en&sort_by=revenue.desc&page=%s" % (
		api_key,
		page_no,
	)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_upcoming(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s"
...
def tmdb_movies_genres(genre_id, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&with_genres=%s&language=en-US&region=US&with_original_language=en&sort_by=popularity.desc&release_date.lte=%s&page=%s"
...
def tmdb_movies_languages(language, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&with_original_language=%s&sort_by=popularity.desc&release_date.lte=%s&page=%s"
...
def tmdb_movies_certifications(certification, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&region=US&with_original_language=en"
...
def tmdb_movies_year(year, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&primary_release_year=%s&page=%s"
...
def tmdb_movies_decade(decade, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&primary_release_date.gte=%s"
...
def tmdb_movies_providers(provider, page_no):
	...
	url = "https://api.themoviedb.org/3/discover/movie?api_key=%s&sort_by=popularity.desc&watch_region=US&with_watch_providers=%s&page=%s" % (
		api_key,
		provider,
		page_no,
	)
...
def tmdb_movies_recommendations(tmdb_id, page_no):
	...
	url = "https://api.themoviedb.org/3/movie/%s/recommendations?api_key=%s&language=en-US&region=US&with_original_language=en&page=%s" % (
		tmdb_id,
		api_key,
		page_no,
	)
...
def tmdb_movies_companies(company_id, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/movie?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&with_companies=%s&page=%s"
...
def tmdb_tv_popular(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en"
...
def tmdb_tv_popular_today(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&without_keywords=210024"
...
def tmdb_tv_premieres(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&language=en-US&region=US&with_original_language=en&include_null_first_air_dates=false"
...
def tmdb_tv_upcoming(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&language=en-US&region=US&with_original_language=en&without_keywords=210024"
...
def tmdb_tv_genres(genre_id, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&with_genres=%s&language=en-US&region=US&with_original_language=en"
...
def tmdb_tv_languages(language, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&language=en-US&include_null_first_air_dates=false&with_original_language=%s&first_air_date.lte=%s"
...
def tmdb_tv_networks(network_id, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&language=en-US&region=US&with_original_language=en&include_null_first_air_dates=false"
...
def tmdb_tv_providers(provider, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&sort_by=popularity.desc&watch_region=US&with_watch_providers=%s&include_null_first_air_dates=false"
...
def tmdb_tv_recommendations(tmdb_id, page_no):
	...
	url = "https://api.themoviedb.org/3/tv/%s/recommendations?api_key=%s&language=en-US&region=US&with_original_language=en&page=%s" % (
		tmdb_id,
		api_key,
		page_no,
	)
...
def tmdb_anime_popular_recent(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&sort_by=first_air_date.desc&include_null_first_air_dates=false&first_air_date_year=%s&page=%s"
...
def tmdb_anime_premieres(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&include_null_first_air_dates=false&first_air_date.gte=%s&first_air_date.lte=%s&page=%s"
...
def tmdb_anime_upcoming(page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&first_air_date.gte=%s&first_air_date.lte=%s&sort_by=first_air_date.asc&page=%s"
...
def tmdb_anime_genres(genre_id, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&with_genres=%s&include_null_first_air_dates=false&first_air_date.lte=%s&page=%s"
		% (api_key, genre_id, get_current_date(), page_no)
	)
	return lists_cache_object(get_data, string, url)
...
def tmdb_anime_providers(provider, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&watch_region=US&with_watch_providers=%s&include_null_first_air_dates=false"
...
def tmdb_anime_year(year, page_no):
	...
	url = "https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&include_null_first_air_dates=false&first_air_date_year=%s&page=%s" % (
		api_key,
		year,
		page_no,
	)
...
def tmdb_anime_decade(decade, page_no):
	...
	url = (
		"https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&include_null_first_air_dates=false&first_air_date.gte=%s"
...
def tmdb_people_full_info(actor_id):
	...
	url = "https://api.themoviedb.org/3/person/%s?api_key=%s&language=en&append_to_response=external_ids,combined_credits,images,tagged_images" % (
		actor_id,
		api_key,
	)
	return cache_function(get_tmdb, string, url, expiration=168)
...
def get_tmdb(url):
	try:
		response = session.get(url, timeout=float(TIMEOUT_STANDARD))
	except:
		response = None
	return response
