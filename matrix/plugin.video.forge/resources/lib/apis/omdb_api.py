...
class OMDbAPI:
	...
	def process_result(self, imdb_id, meta):
		...
		metascore_rating, tomatometer_rating, tomatousermeter_rating = (
			self.process_rating("metascore"),
			self.process_rating("tomatoMeter"),
			self.process_rating("tomatoUserMeter"),
		)
		...
		if tomato_image:
			...
			tomatometer_icon = "rtfresh.png" if int(tomatometer_rating) > 59 else "rtrotten.png"
		else:
			tomatometer_icon = "rtrotten.png"
		if tomatousermeter_rating:
			tomatousermeter_icon = "popcorn.png" if int(tomatousermeter_rating) > 59 else "popcorn_spilt.png"
		else:
	...
	def get_result(self, imdb_id):
		try:
			result = session.get("http://www.omdbapi.com/?apikey=%s&i=%s&tomatoes=True&r=xml" % (self.api_key, imdb_id), timeout=TIMEOUT_STANDARD).text
