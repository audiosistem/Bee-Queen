...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting("forge.rd.client_id", "empty_setting")
		if self.client_ID in ("empty_setting", ""):
			self.client_ID = "X245A4XAIBGVM"
		url = {"true": "app.real-debrid.com", "false": "api.real-debrid.com"}[get_setting("forge.rd.alternate_base_url", "false")]
		self.base_url = "https://%s/rest/1.0/" % url
		self.auth_url = "https://%s/oauth/v2/" % url
		self.token = get_setting("forge.rd.token", "empty_setting")
		self.secret = get_setting("forge.rd.secret", "empty_setting")
		self.refresh = get_setting("forge.rd.refresh", "empty_setting")
		self.device_code = ""
