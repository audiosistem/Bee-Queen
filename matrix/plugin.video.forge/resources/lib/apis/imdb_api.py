...
import requests
from apis._http import TIMEOUT_FAST, TIMEOUT_MEDIUM, TIMEOUT_STANDARD
from caches.base_cache import connect_database
from caches.main_cache import cache_object
from caches.settings_cache import get_setting
from modules.dom_parser import parseDOM
from modules.kodi_utils import logger, sleep
from modules.utils import normalize, remove_accents, replace_html_codes
...
def get_imdb(params):
	...
	if action == "imdb_extras":
		...
		result = requests.post(GQL_URL, json=data, headers=GQL_HEADERS, timeout=TIMEOUT_MEDIUM)
			...
			for i in sorted(result["reviews"]["edges"], key=lambda k: k["node"]["submissionDate"], reverse=True):
				try:
					...
					try:
						...
						rating = "-"
					try:
						title = i["node"]["summary"]["originalText"]
					except Exception:
						title = "-----"
					try:
						date = i["node"]["submissionDate"]
					except Exception:
						date = "-----"
					review = "[B]%02d. [I]%s/10 - %s - %s[/I][/B][CR][CR]%s" % (count, rating, date, title, content)
					if spoiler:
						review = "[B][COLOR red][CONTAINS SPOILERS][/COLOR][CR][/B]" + review
					count += 1
		...
		try:
			...
			for i in sorted(result["trivia"]["edges"], key=lambda k: k["node"]["interestScore"]["usersVoted"], reverse=True):
				try:
					trivia.append("[B]TRIVIA %02d.[/B][CR][CR]%s" % (count, unescape(_clean(i["node"]["displayableArticle"]["body"]["plaidHtml"]))))
				except Exception:
					pass
				count += 1
		except Exception:
		...
		try:
			...
			for i in sorted(result["goofs"]["edges"], key=lambda k: k["node"]["interestScore"]["usersVoted"], reverse=True):
				try:
					blunders.append("[B]BLUNDERS %02d.[/B][CR][CR]%s" % (count, unescape(_clean(i["node"]["displayableArticle"]["body"]["plaidHtml"]))))
				except Exception:
					pass
				count += 1
		except Exception:
		...
		try:
			...
			result = requests.post(GQL_URL, json=payload, headers=GQL_HEADERS, timeout=TIMEOUT_MEDIUM).json()
		...
		try:
			name = params["name"]
			result = requests.get(params["url"], timeout=TIMEOUT_STANDARD)
		...
		if not imdb_list:
			try:
				result = requests.get(params["url_backup"], timeout=TIMEOUT_STANDARD, headers=headers)
		...
		try:
			result = requests.get(params["url"], timeout=TIMEOUT_FAST).json()
...
def clear_imdb_cache(silent=False):
	try:
		...
		logger("IMDb clear_imdb_cache error", str(e))
		return False
...
def refresh_imdb_meta_data(imdb_id):
	try:
		...
		logger("IMDb refresh_imdb_meta_data error", str(e))
		return False
