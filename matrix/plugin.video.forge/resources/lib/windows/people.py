...
from apis.tmdb_api import tmdb_people_full_info, tmdb_people_info
from indexers import dialogs
from indexers.images import Images
from modules.kodi_utils import addon_fanart, execute_builtin, get_icon, hide_busy_dialog, notification, show_busy_dialog
from modules.settings import easynews_authorized, extras_enable_scrollbars, mpaa_region, rpdb_info, tmdb_api_key
...
class People(BaseDialog):
	...
	def onAction(self, action):
		...
		if action == self.info_action:
			...
			if focus_id not in (People.more_from_movies_id, People.more_from_tvshows_id, People.more_from_director_id):
				return
			show_busy_dialog()
		...
		if action in self.selection_actions:
			...
			if self.control_id in (People.more_from_movies_id, People.more_from_tvshows_id, People.more_from_director_id):
				if self.control_id in (People.more_from_movies_id, People.more_from_director_id):
					media_type = "movie"
				else:
					media_type = "tvshow"
	...
	def make_person_data(self):
		...
		try:
			self.person_gender = {0: "", 1: "Female", 2: "Male", 3: ""}[person_info.get("gender")]
		except:
			self.person_gender = ""
		...
		if self.person_deathday:
			self.person_age = calculate_age(self.person_birthday, "%Y-%m-%d", self.person_deathday)
	...
	def make_more_from(self, media_type):
		...
		try:
			if media_type == "movie":
				...
				try:
					data = [i for i in data if 99 not in i["genre_ids"] and i["character"].lower() not in roles_exclude]
				except:
					pass
			elif media_type == "tvshow":
				list_type, _id, data, date_key = media_type, People.more_from_tvshows_id, self.tvshow_data, "first_air_date"
				try:
					data = [i for i in data if not any(x in (10763, 10764, 10767) for x in i["genre_ids"]) and i["character"].lower() not in roles_exclude]
				except:
					pass
			else:
				list_type, _id, data, date_key = "movie", People.more_from_director_id, self.director_data, "release_date"
		...
		for item in data:
			try:
				...
				listitem.setProperties(
					{"name": item[name_key], "release_date": year, "vote_average": "%.1f" % item["vote_average"], "thumbnail": poster, "tmdb_id": str(tmdb_id)}
				)
				append(tmdb_id)
				yield listitem
			except:
	...
	def sort_items_by_release(self, data, key):
		...
		data = [i for i in data if i not in blank]
	...
	def show_text_media(self, text="", poster=None, current_index=None):
		return self.open_window(
			("windows.extras", "ShowTextMedia"), "textviewer_media.xml", text=text, poster=poster or self.person_image, current_index=current_index
		)
	...
	def set_current_params(self, set_starting_position=True):
		self.current_params = {
			...
			"reference_tmdb_id": self.reference_tmdb_id,
			"is_external": self.is_external,
		}
