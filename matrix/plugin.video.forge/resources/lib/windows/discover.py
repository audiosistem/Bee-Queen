...
from modules.kodi_utils import confirm_dialog, container_refresh, get_icon, kodi_dialog, ok_dialog, select_dialog, sleep
...
class Discover(BaseDialog):
	...
	def make_menu(self):
		def builder():
			...
			for key, values in d_items:
				...
				try:
					listitem.setProperty("label2", self.get_attribute(self, values["display_key"]))
				except:
					pass
				try:
	...
	def years(self):
		...
		if choice != None:
			self.set_key_values(self.chosen_item["url_insert_%s" % self.media_type] % str(choice["id"]), str(choice["id"]))
	...
	def genres(self):
		...
		if self.chosen_item["key"] == "with_genres" and self.without_genres_display:
			genres = [i for i in genres if i["name"] not in self.without_genres_display.split(", ")]
		elif self.with_genres_display:
			genres = [i for i in genres if i["name"] not in self.with_genres_display.split(", ")]
		choice = self.multiselect_dialog(self.chosen_item["label"], [{"name": i["name"], "icon": get_icon("genres")} for i in genres], genres)
		if choice != None:
			self.set_key_values(self.chosen_item["url_insert"] % ",".join([i["id"] for i in choice]), ", ".join([i["name"] for i in choice]))
	...
	def network(self):
		...
		if choice != None:
			self.set_key_values(self.chosen_item["url_insert"] % str(choice["id"]), str(choice["name"]))
	...
	def released(self):
		if not self.get_attribute(self, self.chosen_item["display_key"]):
			self.set_key_values(self.chosen_item["url_insert_%s" % self.media_type] % "[current_date]", "True")
		else:
			self.set_key_values("", "")

	def adult(self):
		if not self.get_attribute(self, self.chosen_item["display_key"]):
			self.set_key_values(self.chosen_item["url_insert"] % "true", "True")
		else:
			self.set_key_values("", "")

	def selection_action(self):
		current_value = self.get_attribute(self, self.chosen_item["display_key"])
		if not current_value or self.chosen_item["key"] in ("with_released", "with_adult"):
			return True
		clear_value = confirm_dialog(
			heading="Discover",
			text="Value of [B]%s[/B] already exists.[CR]Change current value or Clear current value?" % current_value,
			ok_label="Clear",
			cancel_label="Change",
			default_control=11,
		)
	...
	def set_attributes_status(self, status="false"):
		...
		if status == "true":
			...
			try:
				self.setProperty("url_label", self.url.split("?")[1])
			except:
				pass
