...
class ThumbImageViewer(BaseDialog):
		...
		if action in self.selection_actions:
			...
			if choice:
				if choice == "delete_image":
					...
					if not path:
						return self.notification("No Image Path to Download")
					params = {
					...
					if in_favorites and action == "Remove From Favorites?":
						self.reset_after_favorite_delete(position)
				else:  # exit_image
					return self.close()
	...
	def make_context_menu(self, enable_delete, enable_favorite):
		...
		if enable_delete:
			choices_append(("Delete", "delete_image"))
		else:
			choices_append(("Download File", "download_image"))
		if enable_favorite:
			choices_append(("Favorites Manager", "manage_favorite"))
		if self.current_page > 1:
			choices_append(("Exit Images", "exit_image"))
