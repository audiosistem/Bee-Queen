...
from modules.kodi_utils import confirm_dialog, ok_dialog, select_dialog
...
class DownloadsManager(BaseDialog):
	...
	def onAction(self, action):
		if action in self.closing_actions:
			...
			if action in self.selection_actions:
				return self.set_status(self.get_listitem(self.window_id))
	...
	def set_status(self, list_item):
		...
		if status == "queued":
			...
			choices.append(("Resume Download", "unpaused"))
		else:
			choices.append(("Pause Download", "paused"))
		...
		if status_choice == None:
			return
		if status_choice == "filename":
			return ok_dialog(text=list_item.getProperty("name"))
		if status_choice == "cancelled" and not confirm_dialog():
			return
		return self.set_home_property("download_status.%s" % item_id, status_choice)

	def monitor(self):
		while not self.closed:
			if not self.active_downloads and not self.active_queued_downloads:
				break
			self.sleep(2500)
