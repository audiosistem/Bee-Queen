...
from modules.kodi_utils import addon_fanart, get_icon, hide_busy_dialog, kodi_dialog, notification, ok_dialog, select_dialog
from modules.settings import avoid_episode_spoilers, max_threads, provider_sort_ranks
...
class SourcesResults(BaseDialog):
		...
		if action in self.selection_actions:
			...
			if filter_type in ("quality", "provider"):
				if filter_value == self.prerelease_key:
					filtered_list = [i for i in self.item_list if i.getProperty(filter_type) in self.prerelease_values]
				else:
					filtered_list = [i for i in self.item_list if i.getProperty(filter_type) == filter_value]
		...
		if action == self.info_action:
			...
			if "Uncached" in chosen_source.get("cache_provider", ""):
				...
				return manual_add_magnet_to_cloud(
					{"mode": "manual_add_magnet_to_cloud", "provider": chosen_source["debrid"], "magnet_url": chosen_source["url"]}
				)
			...
			try:
				...
				if scrape_provider == "external":
					...
					if "Uncached" in item["cache_provider"]:
						if "seeders" in item:
							set_properties({"source_type": "UNCACHED (%d SEEDERS)" % get("seeders", 0)})
						else:
						...
						if pack:
							set_properties({"source_type": "%s [B]PACK[/B]" % cache_flag})
						else:
	...
	def make_filter_items(self):
		...
		qualities = [
			...
			if not (i.getProperty("quality") in duplicates or duplicates.add(i.getProperty("quality"))) and not i.getProperty("quality") == ""
		]
		if any(i in self.prerelease_values for i in qualities):
			qualities = [i for i in qualities if i not in self.prerelease_values] + [self.prerelease_key]
		...
		if "CAM/SCR/TELE" in qualities:
			quality_totals["CAM/SCR/TELE"] = len([i for i in self.item_list if i.getProperty("quality") in self.prerelease_values])
		duplicates = set()
		providers = [
			i.getProperty("provider")
			for i in self.item_list
			if not (i.getProperty("provider") in duplicates or duplicates.add(i.getProperty("provider"))) and not i.getProperty("provider") == ""
		]
		...
		if cache_functions_debrid:
			data.append(("Rescrape with External Cache Check [B]%s[/B]" % ("OFF" if self.external_cache_check else "ON"), "special", "cache_check_rescrape"))
		if self.uncached_results:
			data.append(("Show [B]Uncached[/B] Only | [B]%d[/B] Results" % len(self.uncached_results), "special", "showuncached"))
	...
	def context_menu(self, item):
		...
		if provider_source == "torrent":
			browse_pack_params = {"mode": "debrid.browse_packs", "provider": cache_provider, "name": name, "magnet_url": magnet_url, "info_hash": info_hash}
