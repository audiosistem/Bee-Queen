...
class source:
	...
	def _scrape_history(self):
		try:
			...
			for item in my_history:
				...
				if self.folder_query not in folder_name:
					continue
				if self.media_type == "movie":
					if not any(x in normalized for x in year_query_list):
						continue
				elif not source_utils.seas_ep_filter(self.season, self.episode, normalized):
					continue
				item = {"l": item.get("link_dl") or item.get("link"), "s": item["size"], "n": item["filename"], "direct_debrid_link": bool(item.get("link_dl"))}
				if item["n"].replace("/", "").lower() not in [d["n"].replace("/", "").lower() for d in self.scrape_results]:
	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				...
				folder_name = source_utils.clean_title(normalized)
				if not folder_name:
					folder_results_append(item)
				elif self.folder_query not in folder_name:
					continue
				else:
	...
	def _scrape_folders(self, folder_id):
		try:
			try:
				links = self.AllDebrid.parse_magnet(transfer_id=folder_id)[1]
			except:
				links = []
