...
from modules.settings import easynews_language_filter, filter_by_name
				...
				for item in files:
					try:
						...
						video_quality, details = source_utils.get_file_info(
							name_info=source_utils.release_info_format(file_name), default_quality=self._quality_estimate(int(item.get("width", 0)))
						)
						source_item = {
							"name": file_name,
