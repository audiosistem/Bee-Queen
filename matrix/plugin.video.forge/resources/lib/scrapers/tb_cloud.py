...
class source:
	...
	def _scrape_cloud(self):
		try:
			...
			try:
				my_cloud_files = TorBox.user_cloud()
			except:
				return self.sources
			for item in my_cloud_files["data"]:
				if not item["download_finished"]:
					continue
				if self.folder_query not in source_utils.clean_title(normalize(item["name"])):
					continue
				folder_id = item["id"]
				for file in item["files"]:
					if not file["short_name"].endswith(tuple(self.extensions)):
						continue
	...
	def _scrape_cloud_usenet(self):
		try:
			...
			year_query_list = self._year_query_list()
			try:
				my_cloud_files_usenet = TorBox.user_cloud_usenet()
			except:
				return self.sources
