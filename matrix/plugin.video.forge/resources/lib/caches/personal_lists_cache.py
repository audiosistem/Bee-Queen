...
class PersonalListsCache:
	def make_list(self, list_name, author, sort_order, description, seen="false", poster="", fanart=""):
		try:
			...
			dbcon.execute(
				"INSERT OR REPLACE INTO personal_lists VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				(list_name, json.dumps([]), 0, time_stamp, sort_order, description, seen, poster, fanart, author, time_stamp),
			)
			return True
		except:
			return False
	...
	def delete_list_contents(self, list_name, author):
		try:
			dbcon = connect_database("personal_lists_db")
			dbcon.execute("UPDATE personal_lists SET contents=?, total=? WHERE name=? AND author=?", (json.dumps([]), "0", list_name, author))
			return True
		except:
			return False
	...
	def get_lists(self):
		try:
			dbcon = connect_database("personal_lists_db")
			all_lists = dbcon.execute(
				"SELECT name, total, created, sort_order, description, seen, poster, fanart, author, updated FROM personal_lists"
			).fetchall()
			return [
				{
					"name": str(i[0]),
	...
	def get_list(self, list_name, author, update_seen=True, seen="true", dbcon=None):
		...
		try:
			if not dbcon:
				dbcon = connect_database("personal_lists_db")
			content = _decode(dbcon.execute("SELECT contents FROM personal_lists WHERE name=? AND author=?", (list_name, author)).fetchone()[0])
			if update_seen and self.new_list_check(seen):
				self.update_single_detail("seen", "true", list_name, author)
		except:
			pass
		return content
		...
		try:
			...
			if action == "add":
				if [str(i["media_id"]) for i in contents if str(new_contents["media_id"]) == str(i["media_id"])]:
					return "Item Already in [B]%s[/B]" % list_name
				...
				if not [str(i["media_id"]) for i in contents if str(new_contents) == str(i["media_id"])]:
					return "Item Not in [B]%s[/B]" % list_name
				command = "UPDATE personal_lists SET contents=?, total=total-1, updated=? WHERE name=?"
				contents = [i for i in contents if not str(i["media_id"]) == str(new_contents)]
			dbcon.execute(command, (json.dumps(contents), get_timestamp(), list_name))
			return "Success"
		except:
			return "Error"
	...
	def add_many_list_items(self, list_name, author, new_contents):
		try:
			...
			dbcon.execute(
				"UPDATE personal_lists SET contents=?, total=?, updated=? WHERE name=? AND author=?",
				(json.dumps(contents), len(contents), get_timestamp(), list_name, author),
			)
			return "Success"
		except:
			return "Error"
