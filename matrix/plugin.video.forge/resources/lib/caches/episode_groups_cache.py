...
class EpisodeGroupsCache:
	def get(self, tmdb_id):
		try:
			data = _decode(connect_database("episode_groups_db").execute("SELECT data FROM groups_data WHERE tmdb_id = ?", (str(tmdb_id),)).fetchone()[0])
		except:
			data = {}
		return data

	def set(self, tmdb_id, data):
		connect_database("episode_groups_db").execute("INSERT OR REPLACE INTO groups_data VALUES (?, ?)", (str(tmdb_id), json.dumps(data)))
