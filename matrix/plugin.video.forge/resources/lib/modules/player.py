...
class ForgePlayer(xbmc.Player):
	...
	def media_watched_marker(self, force_watched=False):
		...
		try:
			if self.current_point >= 90 or force_watched:
				...
				ku.clear_property("forgepisode_history")
	...
	def _resolve_skip_segments(self):
		...
		try:
			...
			ku.logger("Forge.player", "skip segment resolve failed: %s" % error)
			self.skip_segments = []
	...
	def _run_skip_button(self, seg):
		try:
			...
			ku.logger("Forge.player", "skip button failed: %s" % error)
	...
	def clear_playback_properties(self):
		ku.clear_property("forgendow_stack")
