...
from apis.tmdb_api import (
	episode_group_details,
	episode_groups_data,
	movie_details,
	movie_external_id,
	movie_set_details,
	season_episodes_details,
	tvshow_details,
	tvshow_external_id,
)
...
def movie_meta(id_type, media_id, api_key, mpaa_region, current_date, current_time=None):
	...
	try:
		if id_type in ("tmdb_id", "imdb_id"):
			data = movie_details(media_id, api_key)
		else:
			external_result = movie_external_id(id_type, media_id, api_key)
			if not external_result:
				data = None
			else:
				data = movie_details(external_result["id"], api_key)
		if not data:
			return None
		elif data.get("status_code") in (6, 34, 37):
			if id_type == "tmdb_id":
				meta = {"tmdb_id": media_id, "imdb_id": "tt0000000", "tvdb_id": "0000000", "blank_entry": True}
			else:
				meta = {"tmdb_id": "0000000", "imdb_id": media_id, "tvdb_id": "0000000", "blank_entry": True}
		...
		if poster_path:
			poster = tmdb_image_url % ("w780", poster_path)
		else:
			poster = ""
		backdrop_path = data_get("backdrop_path", "")
		if backdrop_path:
			fanart = tmdb_image_url % ("w1280", backdrop_path)
		else:
			fanart = ""
		...
		if images:
			try:
				...
				if not clearlogo.endswith("png"):
					clearlogo = clearlogo.replace(clearlogo.split(".")[-1], "png")
			except:
				clearlogo = ""
			try:
				...
				if logo_path.endswith("png"):
					...
					clearlogo = tmdb_image_url % ("original", logo_path.replace(logo_path.split(".")[-1], "png"))
			except:
				clearlogo = ""
			try:
				landscape = next((tmdb_image_url % ("w1280", i["file_path"]) for i in images["backdrops"] if i["iso_639_1"] == "en"), "")
			except:
				landscape = ""
			if not poster:
				poster = next((tmdb_image_url % ("w780", i["file_path"]) for i in images["posters"] if i["iso_639_1"] == "en"), "")
			if not fanart:
				fanart = next((tmdb_image_url % ("w1280", i["file_path"]) for i in images["backdrops"] if i["iso_639_1"] in (None, "xx")), "")
		else:
			clearlogo, landscape = "", ""
		...
		if companies:
			if len(companies) == 1:
				...
				try:
					studio = (next(i["name"] for i in companies if i["logo_path"] not in ("", "None", None)) or next(i["name"] for i in companies),)
				except:
					pass
		...
		if release_dates:
			...
			try:
				mpaa = next(
					x["certification"] for i in release_results for x in i["release_dates"] if i["iso_3166_1"] == mpaa_region and x["certification"] != ""
				)
			except:
				pass
		...
		if credits:
			...
			if all_cast:
				try:
					cast = [
						{"name": i["name"], "role": i["character"], "thumbnail": tmdb_image_url % ("h632", i["profile_path"]) if i["profile_path"] else ""}
						for i in all_cast
					]
					short_cast = cast[:10]
				except:
			...
			if crew:
				try:
					writer = [i["name"] for i in crew if i["job"] in ("Author", "Writer", "Screenplay", "Characters")]
				except:
					pass
				try:
					...
					trailer = (
						next(
							(
								...
								if i["official"] and i["type"] == "Trailer" and "official trailer" in i["name"].lower()
							),
							None,
						)
						or next((youtube_url % i["key"] for i in all_trailers if i["official"] and i["type"] == "Trailer"), None)
						or next((youtube_url % i["key"] for i in all_trailers if i["type"] == "Trailer"), None)
						or next((youtube_url % i["key"] for i in all_trailers if "trailer" in i["name"].lower()), None)
						or next((youtube_url % i["key"] for i in all_trailers), None)
						or ""
					)
				else:
					trailler = ""
		...
		meta = {
			...
			"landscape": landscape,
			"keywords": keywords,
			"rpdb_poster": rpdb_poster,
			"short_cast": short_cast,
			"stinger_keys": stinger_keys,
		}
...
def tvshow_meta(id_type, media_id, api_key, mpaa_region, current_date, current_time=None, is_anime_list=None):
	...
	try:
		if id_type == "tmdb_id":
			data = tvshow_details(media_id, api_key)
		else:
			external_result = tvshow_external_id(id_type, media_id, api_key)
			if not external_result:
				data = None
			else:
				data = tvshow_details(external_result["id"], api_key)
		if not data or data.get("status_code", "") in (6, 34, 37):
			if id_type == "tmdb_id":
				...
				meta = {"tmdb_id": "0000000", "imdb_id": "tt0000000", "tvdb_id": media_id, "blank_entry": True}
		...
		if images:
			...
			if not poster:
				poster = next((tmdb_image_url % ("w780", i["file_path"]) for i in images["posters"] if i["iso_639_1"] == "en"), "")
			if not fanart:
				fanart = next((tmdb_image_url % ("w1280", i["file_path"]) for i in images["backdrops"] if i["iso_639_1"] == "xx"), "")
		else:
			clearlogo, landscape = "", ""
		...
		if networks:
			if len(networks) == 1:
				...
				try:
					studio = (next(i["name"] for i in networks if i["logo_path"] not in ("", "None", None)) or next(i["name"] for i in networks),)
				except:
					pass
		production_countries = data_get("production_countries", None)
		if production_countries:
		...
		if content_ratings:
			try:
				mpaa = next((i["rating"] for i in content_ratings["results"] if i["iso_3166_1"] == mpaa_region), "")
			except:
				pass
		credits = data_get("credits")
		if credits:
		...
		if ei_last_ep and status not in ("Ended", "Canceled"):
			total_aired_eps = (
				sum([i["episode_count"] for i in season_data if i["season_number"] < ei_last_ep["season_number"] and i["season_number"] != 0])
		...
		extra_info = {
			...
			"next_episode_to_air": ei_next_ep,
...
def episodes_meta(season, meta):
	def _process():
		...
		for ep_data in details:
			...
			if still_path:
				thumb = tmdb_image_url % ("original", still_path)
			else:
				thumb = None
	...
	try:
		...
		if season == 1:
			season_type = "premiere_finale" if (total_seasons == season and tvshow_status in ("Ended", "Canceled")) else "premiere"
		else:
			season_type = "finale" if (total_seasons == season and tvshow_status in ("Ended", "Canceled")) else ""
		if tvshow_status in ("Ended", "Canceled") or total_seasons > int(season):
			expiration = 4368
		else:
			expiration = 96
		try:
...
def all_episodes_meta(meta, include_specials=False):
	...
	def _get_tmdb_episodes(season):
		try:
			data.extend(episodes_meta(season, meta))
		except:
			pass
...
def episode_groups(media_id):
	try:
		groups = episode_groups_data(media_id)["results"]
	except:
		groups = None
...
def group_episode_data(details, episode_id=None, season_number=None, episode_number=None):
	def _comparer(episode_item):
		if episode_id:
			return episode_item["id"] == int(episode_id)
		else:
			return episode_item["season_number"] == int(season_number) and episode_item["episode_number"] == int(episode_number)
...
def tvshow_expiry(current_date, meta):
	try:
		if meta["status"] in ("Ended", "Canceled"):
			expiration = 4368
		else:
...
def meta_valid_check(meta, is_anime_list):
	if is_anime_list == None:
		return meta
	if is_anime_check(meta) != is_anime_list:
		meta = {}
	return meta
...
def is_anime_check(meta=None, tmdb_id=None):
	if not meta:
		meta = meta_cache.get("tvshow", "tmdb_id", tmdb_id)
	try:
