...
class MenuEditor:
	...
	def _icon_select(self, default_icon=""):
		if default_icon.startswith("http") or "plugin.video.forge" in default_icon:
			return default_icon
	...
	def _path_browser(self, label="", file="plugin://plugin.video.forge?mode=navigator.main&full_list=true", thumbnail=""):
		kodi_utils.show_busy_dialog()
		results = kodi_utils.jsonrpc_get_directory(file)
		kodi_utils.hide_busy_dialog()
		list_items, function_items = [], []
		if file != "plugin://plugin.video.forge?mode=navigator.main&full_list=true":
	...
	def _get_menu_item(self, path):
		return dict(parse_qsl(path.replace("plugin://plugin.video.forge/?", "")))
