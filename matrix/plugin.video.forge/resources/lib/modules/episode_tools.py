	...
	def get_random_episode(self, continual=False, first_run=True):
		try:
			...
			if continual:
				episode_list = []
				try:
					episode_history = json.loads(kodi_utils.get_property("forge.random_episode_history"))
					if tmdb_key in episode_history:
						episode_list = episode_history[tmdb_key]
					else:
						kodi_utils.set_property("forge.random_episode_history", "")
				except:
					kodi_utils.set_property("forge.random_episode_history", "")
			...
			if continual:
				...
				kodi_utils.set_property("forge.random_episode_history", json.dumps(episode_history))
