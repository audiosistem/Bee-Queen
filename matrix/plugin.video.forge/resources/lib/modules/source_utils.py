...
def internal_results(provider, sources):
	set_property("forge.internal_results.%s" % provider, json.dumps(sources))
...
def get_external_cache_status(debrid, unchecked_hashes, data, active_debrid):
	...
	try:
		...
		debrid_name, services, token = {
			"Real-Debrid": ("realdebrid", ["torrentio"], get_setting("forge.rd.token")),
			"AllDebrid": ("alldebrid", ["mediafusion"], get_setting("forge.ad.token")),
		}[debrid]
