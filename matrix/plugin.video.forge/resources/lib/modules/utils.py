...
from modules.kodi_utils import logger, sleep
	...
	def _thread_target(self, queue, target):
		while not queue.empty():
			try:
				...
				logger("thread queue error", str(e))
...
def version_tuple(version):
	...
	Defensive so a malformed stored version never crashes the migration runner:
	  ''          -> (0,)          (sorts lowest)
	  'v1.2-beta' -> (1, 2)        (non-digits stripped per component)
	  '0.10.0'    -> (0, 10, 0)    (correctly greater than (0, 9, 0))
	Tuple ordering is lexicographic, so (0, 2) < (0, 2, 1) as expected (a
	missing patch sorts before any explicit patch).
	"""
	if not version:
		return (0,)
	parts = []
...
def clean_file_name(s, use_encoding=False, use_blanks=True):
	try:
		...
		special_encoded = [['"', "%22"], ["*", "%2A"], ["/", "%2F"], [":", ","], ["<", "%3C"], [">", "%3E"], ["?", "%3F"], ["\\", "%5C"], ["|", "%7C"]]
...
def sort_for_article(_list, _key, ignore_articles):
	try:
		if not ignore_articles:
			...
			_list.sort(key=lambda k: re.sub(r"(^the |^a |^an )", "", k.get(_key).lower()))
	except:
		pass
	return _list
...
def unzip(zip_location, destination_location, destination_check, show_busy=True):
	...
	from modules.kodi_utils import hide_busy_dialog, path_exists, show_busy_dialog
...
def make_qrcode(url):
	...
	try:
		# Unique filename per URL: Kodi's GUI texture cache keys on the file path,
		# so a fixed name (qr.png) would serve a stale cached QR from a prior service.
		digest = md5(url.encode("utf-8")).hexdigest()[:12]
		art_path = path.join(addon_profile(), "qr_%s.png" % digest)
...
def launch_browser(url):
	# Open url in the OS default browser (Windows: default browser, Linux: xdg-open,
	# macOS: open). Returns True/False; never raises, so callers can offer it as a
	# best-effort convenience and fall back where no browser exists.
	try:
		import webbrowser
...
def image_from_db(image_url, delete=True):
	...
	from modules.kodi_utils import notification, translate_path
	...
	try:
		...
		try:
			image_id, image_location = dbcur.execute("SELECT id, cachedurl FROM texture WHERE url = ?", (image_url,)).fetchone()
		except:
			return True
...
def make_image(list_type, image_type, list_name, images, current_image):
	...
	from modules.kodi_utils import addon_profile, make_directory, notification, translate_path
	...
	try:
		...
		if image_type == "poster":
			new_dimensions, size_dimensions, placements = (1000, 1500), (500, 750), ((0, 0), (500, 0), (0, 750), (500, 750))
		else:
			new_dimensions, size_dimensions, placements = (1280, 720), (640, 360), ((0, 0), (640, 0), (0, 360), (640, 360))
