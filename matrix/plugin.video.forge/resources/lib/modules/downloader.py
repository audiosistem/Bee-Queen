...
def download_threads_manager(multi_downloads, image):
	...
	for item in multi_downloads:
		...
		kodi_utils.set_property("forge.active_queued_downloads", json.dumps(remaining_downloads))
	kodi_utils.clear_property("forge.active_queued_downloads")
...
class Downloader:
	...
	def get_active_downloads(self):
		return json.loads(kodi_utils.get_property("forge.active_downloads") or "[]")
	...
	def add_active_download(self):
		...
		kodi_utils.set_property("forge.active_downloads", json.dumps(active_downloads))
	...
	def clear_active_downloads(self):
		kodi_utils.clear_property("forge.active_downloads")

	def set_percent_property(self, percent):
		kodi_utils.set_property("forge.%s" % self.final_name, str(percent))

	def check_status(self):
		status = kodi_utils.get_property("forge.download_status.%s" % self.final_name)
		if status in ("unpaused", "cancelled"):
			kodi_utils.clear_property("forge.download_status.%s" % self.final_name)
		return status
