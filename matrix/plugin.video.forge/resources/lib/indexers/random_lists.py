...
class RandomLists:
	...
	def random_because_you_watched(self):
		...
		try:
			...
			kodi_utils.clear_property("forge.random_because_you_watched")
		self.make_directory()
	...
	def make_directory(self, next_page_params={}):
		...
		if self.is_external:
			if self.folder_name:
				kodi_utils.set_property("forge.%s" % self.folder_name, self.category_name)
			else:
				kodi_utils.set_property("forge.%s" % self.base_list_name, self.category_name)
		else:
...
def random_shortcut_folders(folder_name, random_results):
	...
	if is_external:
		kodi_utils.set_property("forge.%s" % folder_name, list_name)
	if menu_type == "movie":
