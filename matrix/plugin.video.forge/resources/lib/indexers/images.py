...
from apis.tmdb_api import tmdb_media_images, tmdb_people_full_info, tmdb_people_info, tmdb_popular_people, tmdb_trending_people_day, tmdb_trending_people_week
from modules.kodi_utils import delete_file, get_icon, hide_busy_dialog, image_extensions, list_dirs, make_listitem, notification, show_busy_dialog
...
class Images:
	def run(self, params):
		...
		if self.mode == "imageviewer":
			return self.imageviewer()
		if self.mode == "delete_image":
			return self.delete_image()
		if self.mode == "tmdb_people_list_image_results":
			self.tmdb_people_list_image_results()
		elif self.mode == "tmdb_people_search_image_results":
			self.tmdb_people_search_image_results()
		elif self.mode == "favorite_people_list_image_results":
			self.favorite_people_list_image_results()
		elif self.mode == "tmdb_media_image_results":
			self.tmdb_media_image_results()
		elif self.mode == "tmdb_people_image_results":
			self.tmdb_people_image_results()
		elif self.mode == "tmdb_people_tagged_image_results":
			self.tmdb_people_tagged_image_results()
		elif self.mode == "favorites_image_results":
			self.favorites_image_results()
		elif self.mode == "easynews_image_results":
			self.easynews_image_results()
		elif self.mode == "browser_image":
			self.browser_image()
		hide_busy_dialog()
		if self.direct_search_result:
			return
		if len(self.list_items) == 0:
			return notification("None Found")
		if "in_progress" not in params:
	...
	def tmdb_media_image_results(self):
		...
		try:
			...
			fanarts, landscapes = (
				[i for i in results["backdrops"] if i["iso_639_1"] in (None, "xx")],
				[i for i in results["backdrops"] if i["iso_639_1"] == "en"],
			)
			for item in ((posters, "%s _Poster_%03d"), (fanarts, "%s _Fanart_%03d"), (landscapes, "%s _Landscape_%03d"), (clearlogos, "%s _Clearlogo_%03d")):
				if item[0]:
					all_images.extend(
						[
							(self.tmdb_image_base % ("original", i["file_path"]), item[1] % (rootname, count), self.tmdb_image_base % ("w300", i["file_path"]))
	...
	def tmdb_people_image_results(self):
		...
		try:
			...
			all_images = [
				(self.tmdb_image_base % ("original", i["file_path"]), "%s_%03d" % (actor_name, count), self.tmdb_image_base % ("w185", i["file_path"]))
				for count, i in enumerate(image_info, 1)
			]
		except:
			pass
		image_action = json.dumps({"mode": "imageviewer", "all_images": all_images})
	...
	def tmdb_people_tagged_image_results(self):
		...
		image_info.sort(key=lambda x: x["file_path"])
		all_images = [
			(self.tmdb_image_base % ("original", i["file_path"]), (i["media"]["title"]) if "title" in i["media"] else i["media"]["name"]) for i in image_info
		]
		image_action = json.dumps({"mode": "imageviewer", "all_images": all_images})
		self.list_items = list(builder())
		...
		def builder():
			for item in image_info:
				try:
					listitem = make_listitem()
					listitem.setProperties(
						{"thumb": item["thumbnail"], "path": item["down_url"], "name": item["name"], "action": image_action, "image_subject": key_id}
					)
					yield listitem
				except Exception:
					pass
	...
	def imageviewer(self):
		hide_busy_dialog()
		return open_window(
			("windows.imageviewer", "ImageViewer"), "imageviewer.xml", all_images=self.params["all_images"], index=int(self.params["current_index"])
		)
