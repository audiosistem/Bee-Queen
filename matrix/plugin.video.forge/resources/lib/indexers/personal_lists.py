...
def get_personal_lists(params):
	...
	try:
		data = get_all_personal_lists(get_setting("forge.personal_list.list_sort", "0"))
		if data:
			if shuffle_lists:
				returning_to_list = "build_personal_lists_contents" in kodi_utils.folder_path()
				if returning_to_list:
					try:
						data = json.loads(kodi_utils.get_property("forge.personal.lists.order"))
					except:
						pass
				else:
					shuffle(data)
					kodi_utils.set_property("forge.personal.lists.order", json.dumps(data))
			else:
				kodi_utils.clear_property("forge.personal.lists.order")
...
def make_new_personal_list(params):
	...
	if (
		...
		if suggested_author in ("Collection", "Watchlist"):
			suggested_author = get_setting("forge.trakt.user")
