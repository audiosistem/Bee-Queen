		...
		if data:
			if shuffle_lists:
				...
				if returning_to_list:
					try:
						data = json.loads(kodi_utils.get_property("forge.trakt.lists.order"))
					except:
						pass
				else:
					shuffle(data)
					kodi_utils.set_property("forge.trakt.lists.order", json.dumps(data))
			else:
				kodi_utils.clear_property("forge.trakt.lists.order")
