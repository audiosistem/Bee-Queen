...
def window_theme_choice(params):
	if params["type"] == "theme":
		...
		window_theme_opacity = get_setting("forge.window_theme_opacity", "CC")
		...
		window_theme = get_setting("forge.window_theme", "FF1F2020")[2:]
		window_theme_contrast = get_setting("forge.window_theme_contrast", "FF4a4347")
...
def language_invoker_choice(params):
	...
	addon_xml = kodi_utils.translate_path("special://home/addons/plugin.video.forge/addon.xml")
...
def personallists_manager_choice(params):
	...
	list_type = params["list_type"]
	all_lists = get_all_personal_lists(get_setting("forge.personal_list.list_sort", "0"))
	choices = []
	if not all_lists:
		action = "add_new"
	else:
...
def quick_add_target_choice(params):
	...
	kinds = [
		...
		("Forge Favorites", "forge_favorites"),
		("Trakt Personal List...", "pick_trakt_list"),
		("TMDb List...", "pick_tmdb_list"),
		("Forge Personal List...", "pick_forge_list"),
	]
	...
	if choice in quick_add.STATIC_KINDS:
		...
		if not selection:
			return
		token, label = quick_add.encode_target("forge_list", name=selection[0], author=selection[1]), selection[0]
...
def set_quality_choice(params):
	...
	fl = ["4K", "1080p", "720p", "SD"]
	q_setting = get_setting("forge.%s" % quality_setting).split(", ")
	try:
	...
	if not orig_button_dict:
		for _type in ("movie", "tvshow"):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting("forge.%s" % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace("forge.", ""), default_setting_values(setting_id)["setting_default"])
					button_action = get_setting("forge.%s" % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def extras_ratings_choice(params={}):
	...
	list_items = [{"line1": i[0], "icon": "forge_flags/ratings/%s" % i[2]} for i in choices]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting("forge.%s" % filter_setting_id).split(", ")
	try:
...
def enable_scrapers_choice(params={}):
	icon = params.get("icon", None) or kodi_utils.get_icon("forge")
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting("forge.%s" % setting), default_setting_values(setting)["setting_default"]
