...
from modules.kodi_utils import add_items, build_url, end_directory, get_addon_fanart, get_icon, make_listitem, set_category, set_content
...
def person_data_dialog(params):
	if "key_id" in params:
		key_id = unquote(params.get("key_id"))
	elif "query" in params:
		key_id = unquote(params.get("query"))
	else:
		key_id = None
	open_window(
		...
		reference_tmdb_id=params.get("reference_tmdb_id"),
	...
	def _builder():
		for item in data:
			...
			if item["profile_path"]:
				actor_image = "https://image.tmdb.org/t/p/%s%s" % ("h632", item["profile_path"])
			else:
				actor_image = icon
