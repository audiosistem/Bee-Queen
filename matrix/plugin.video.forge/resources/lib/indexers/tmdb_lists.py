...
def get_tmdb_lists(params):
	...
	try:
		data = get_all_tmdb_lists(get_setting("forge.tmdblist.list_sort", "0"))
		if data:
			if shuffle_lists:
				returning_to_list = "build_tmdb_lists_contents" in kodi_utils.folder_path()
				if returning_to_list:
					try:
						data = json.loads(kodi_utils.get_property("forge.tmdb.lists.order"))
					except:
						pass
				else:
					shuffle(data)
					kodi_utils.set_property("forge.tmdb.lists.order", json.dumps(data))
			else:
				kodi_utils.clear_property("forge.tmdb.lists.order")
