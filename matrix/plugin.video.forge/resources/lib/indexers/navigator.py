...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get("name", "Forge")
	...
	def tools(self):
		...
		if get_setting("forge.external_scraper.module") not in ("empty_setting", ""):
		...
		if get_setting("forge.use_viewtypes", "true") == "true" and not get_setting("forge.manual_viewtypes", "false") == "true":
	...
	def changelog_utils(self):
		...
		forge_clogpath = k.translate_path("special://home/addons/plugin.video.forge/resources/text/changelog.txt")
		self.add({"mode": "show_text", "heading": "Changelog", "file": forge_clogpath, "font_size": "large", "isFolder": "false"}, "Changelog", "lists")
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith("http") or "plugin.video.forge" in iconImage:
						original_image = True
					else:
	...
	def exit_media_menu(self):
		params = k.get_property("forge.exit_params")
		if params:
			return k.container_refresh_input(params)

	def tips(self):
		tips_location = "special://home/addons/plugin.video.forge/resources/text/tips"
	...
	def in_progress_mixed(self):
		...
		# under Forge's nested thread fan-out: this handler spawned fetch/build threads
		...
		# See docs/ai-docs/changes-from-fenlight.md.
		...
		try:
			...
			page_no = 1
		per_type = 20
		# forge.exit_params drives the context-menu "Exit … List" action only (Navigator.exit_media_menu);
		...
		k.logger("Forge", "in_progress_mixed: page=%s start" % page_no)
		...
		def _fetch_movies():
			try:
				...
				k.logger("Forge", "in_progress_mixed._fetch_movies: %s" % e)

		def _fetch_episodes():
			try:
				episode_data.extend(get_in_progress_episodes() or [])
			except Exception as e:
				k.logger("Forge", "in_progress_mixed._fetch_episodes: %s" % e)
		...
		if not _settings.mixed_include_anime() and episode_data:
			...
			if anime_show_ids:
				...
				k.logger("Forge", "in_progress_mixed: dropped %s anime episode(s)" % (before - len(episode_data)))
		...
		def _build_movies():
			...
			try:
				...
				k.logger("Forge", "in_progress_mixed._build_movies: %s" % e)
		...
		def _build_episodes():
			...
			try:
				...
				k.logger("Forge", "in_progress_mixed._build_episodes: %s" % e)
	...
	def mixed_list(self):
		...
		if not sources:
			k.logger("Forge", "mixed_list: unknown action %s" % action)
			k.end_directory(handle)
			return
		try:
			page_no = max(1, int(self.params_get("new_page", "1")))
		...
		k.logger("Forge", "mixed_list: action=%s page=%s start" % (action, page_no))
		...
		def _fetch(target, fn):
			try:
				...
				k.logger("Forge", "mixed_list._fetch %s: %s" % (getattr(fn, "__name__", "?"), e))
		...
		def _build(bucket_key, builder_cls, ids, menu_type):
			...
			try:
				...
				k.logger("Forge", "mixed_list._build %s: %s" % (bucket_key, e))
	...
	def mixed_keyed_list(self):
		...
		if not sources:
			k.logger("Forge", "mixed_keyed_list: unknown action %s" % action)
			k.end_directory(handle)
			return
		...
		if not movie_key or not tv_key:
			k.logger("Forge", "mixed_keyed_list: missing key for action %s" % action)
			k.end_directory(handle)
			return
		try:
			page_no = max(1, int(self.params_get("new_page", "1")))
		...
		k.logger("Forge", "mixed_keyed_list: action=%s page=%s start" % (action, page_no))
		...
		def _fetch(target, fn, key):
			try:
				...
				k.logger("Forge", "mixed_keyed_list._fetch %s: %s" % (getattr(fn, "__name__", "?"), e))
		...
		def _build(bucket_key, builder_cls, ids, menu_type):
			...
			try:
				...
				k.logger("Forge", "mixed_keyed_list._build %s: %s" % (bucket_key, e))
	...
	def mixed_brand_list(self):
		...
		if not providers and not networks:
			k.logger("Forge", "mixed_brand_list: no providers/networks")
			k.end_directory(handle)
			return
		try:
			page_no = max(1, int(self.params_get("new_page", "1")))
		...
		if page_no == 1 and not self.is_external:
			k.set_property("forge.exit_params", k.folder_path())
		label = self.params_get("name", "Mixed")
		k.logger("Forge", "mixed_brand_list: name=%s page=%s start" % (label, page_no))
		...
		def _fetch(target, fn, key):
			try:
				...
				k.logger("Forge", "mixed_brand_list._fetch %s: %s" % (getattr(fn, "__name__", "?"), e))
		...
		def _build(bucket_key, builder_cls, ids, menu_type):
			...
			try:
				...
				k.logger("Forge", "mixed_brand_list._build %s: %s" % (bucket_key, e))
