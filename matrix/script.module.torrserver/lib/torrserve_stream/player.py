    ...
    def UpdateProgress(self):
        if self.show_overlay:
            ...
            try:
                ...
                if percent >= 0:
                    heading = u"{0} MB din {1} MB - {2}".format(downloaded/1024/1024, size/1024/1024, int(percent)) + r'%' + '\n'
                    if percent < 100:
                        heading += u"Viteză descărcare: {0} KB/s\n".format(dl_speed/1024)
                        heading += u"Seederi: {0}    Peeri: {1}".format(info['ConnectedSeeders'], info['ActivePeers'])
