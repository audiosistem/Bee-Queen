...
class Core():
    ...
    def _process_item(self, data):
        ...
        try:
            ...
            for stream in streams:
                ...
                def is_url_online(url, timeout=5):
                    try:
                        ...
                        self.log(f"Error {e}: {url} ")
                        return False
                if not is_url_online(video_url):
                    continue
            ...
            self.log(f"Error processing source: {e}", "info")

        return sources_list

    def _make_query(self, imdb_id, simple_info, type):
        headers = {'User-Agent': self.user_agent}
        if type == 'TV':
            item_url = f"{self.base_url}series/{imdb_id}:{simple_info['season_number']}:{simple_info['episode_number']}.json"
        else:
            item_url = f"{self.base_url}movie/{imdb_id}.json"
    ...
    def episode(self, simple_info, info):
        ...
        sources = []
        try:
            data = self._make_query(imdb_id, simple_info, "TV")
            source = self._process_item(data)
            for src in source:
                if src is not None:
                    sources.append(src)
            return self._return_results("episode", sources)
    ...
    def movie(self, title, year, imdb, simple_info, info):
        ...
        sources = []
        try:
            data = self._make_query(imdb, simple_info, "Movie")
            source = self._process_item(data)
            for src in source:
                if src is not None:
                    sources.append(src)
