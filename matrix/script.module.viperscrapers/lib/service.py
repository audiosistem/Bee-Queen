...
from viperscrapers.modules.Thread_pool import shutdown_executor
...
class CheckSettingsFile:
	def run(self):
		try:
			xbmc.log('[ script.module.viperscrapers ]  CheckSettingsFile Service Starting...', LOGINFO)
			window.clearProperty('viperscrapers')
			...
			if not control.existsPath(settings_xml):
				control.setSetting('module.provider', 'ViperScrapers')
				xbmc.log('%s : created successfully' % settings_xml, LOGINFO)
			else: xbmc.log('%s : already exists' % settings_xml, LOGINFO)
			return xbmc.log('[ script.module.viperscrapers ]  Finished CheckSettingsFile Service', LOGINFO)
		except:
			import traceback
			traceback.print_exc()
...
class SettingsMonitor(control.monitor_class):
	def __init__ (self):
		...
		window.setProperty('viperscrapers.debug.reversed', control.setting('debug.reversed'))
		xbmc.log('[ script.module.viperscrapers ]  Settings Monitor Service Starting...', LOGINFO)
...
class CheckUndesirablesDatabase:
	def run(self):
		xbmc.log('[ script.module.viperscrapers ]  "CheckUndesirablesDatabase" Service Starting...', LOGINFO)
		from viperscrapers.modules import undesirables
		try:
			...
			import traceback
			traceback.print_exc()
		return xbmc.log('[ script.module.viperscrapers ]  Finished "CheckUndesirablesDatabase" Service', LOGINFO)

def main():
	while not control.monitor.abortRequested():
		xbmc.log('[ script.module.viperscrapers ]  Service Started', LOGINFO)
		CheckSettingsFile().run()
		CheckUndesirablesDatabase().run()
		if control.isVersionUpdate():
			control.clean_settings()
			xbmc.log('[ script.module.viperscrapers ]  Settings file cleaned complete', LOGINFO)
		break
	SettingsMonitor().waitForAbort()
	shutdown_executor()
	xbmc.log('[ script.module.viperscrapers ]  Service Stopped', LOGINFO)
