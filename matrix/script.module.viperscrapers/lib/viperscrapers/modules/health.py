...
default_icon = Addon('script.module.viperscrapers').getAddonInfo('icon')
...
heading, resolutions = 'ViperScrapers', '4K 1080p 720p SD'
...
class Magneto:
	def __init__(self):
		self.data = {'imdb': 'tt1502397', 'title': 'Bad Boys for Life', 'aliases': [], 'year': 2020}
	...
	def health_check(self):
		...
		dialog.close()
		select(heading, items, useDetails=True)

	def build_modules(self):
		for provider, module in viperscrapers.sources(ret_all=True):
	...
	def _make_items(self, modules):
		for i, module in enumerate(modules):
			try:
				...
				icon = self.data['poster'] if not i % 2 else default_icon
				item = list_item(line1, line2, offscreen=True)
				item.setArt({'poster': icon})
				yield item
			except: pass

if __name__ == '__main__': Magneto().health_check()
