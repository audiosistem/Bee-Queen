...
addonObject = addon('script.module.viperscrapers')
...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('viperscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('viperscrapers_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear viperscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('viperscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('viperscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.viperscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('ViperScrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.viperscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.viperscrapers').getAddonInfo('version')
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.viperscrapers'
	try:
		removed_settings = []
		active_settings = []
