...
import inspect
from viperscrapers.modules.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR orange][ ViperScrapers %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'viperscrapers.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.viperscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		if exception:
			...
			addon = 'script.module.viperscrapers'
		...
		import xbmc
		xbmc.log('[ script.module.viperscrapers ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from viperscrapers.modules.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.viperscrapers ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def view_LogFile(name):
	try:
		...
		from viperscrapers.modules.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from viperscrapers.modules.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from viperscrapers.modules.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'ViperScrapers %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('ViperScrapers log file uploaded to: %s' % result)
			...
			if 'Copy url To Clipboard' in list[select][0]:
				from viperscrapers.modules.source_utils import copy2clip
				copy2clip(list[select - 1][1])
		elif 'message' in response.json():
			notification(message='ViperScrapers Log upload failed: %s' % str(response.json()['message']))
			log('ViperScrapers Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='ViperScrapers Log upload failed')
			log('ViperScrapers Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('ViperScrapers log upload failed')
