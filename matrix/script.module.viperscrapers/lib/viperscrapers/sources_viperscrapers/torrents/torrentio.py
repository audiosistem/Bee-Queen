...
from viperscrapers.modules import source_utils, cache
from viperscrapers.modules.control import setting as getSetting, homeWindow, sleep
...
# Currently supports YTS(+), EZTV(+), RARBG(+), 1337x(+), ThePirateBay(+), KickassTorrents(+), TorrentGalaxy(+), HorribleSubs(+), NyaaSi(+), NyaaPantsu(+), Rutor(+), Comando(+), ComoEuBaixo(+), Lapumia(+), OndeBaixa(+), Torrent9(+).
	...
	def sources(self, data, hostDict):
		...
		sources = []
		if not data:
			homeWindow.clearProperty('viperscrapers.torrentio.performing_single_scrape')
			return sources
		...
		try:
			...
			if 'tvshowtitle' in data:
				homeWindow.setProperty('viperscrapers.torrentio.performing_single_scrape', 'true')
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		while count < 10000 and not finished_single_scrape:
			finished_single_scrape = homeWindow.getProperty('viperscrapers.torrentio.performing_single_scrape') != 'true'
			sleep(100)
			count += 100
