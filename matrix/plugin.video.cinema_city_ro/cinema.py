...
def list_content(content_type, page=1, query=None):
    ...
    for item in results:
        ...
        mode = 'play' if content_type == 'movie' else 'details'
        add_directory(f"{title} ({year})", 
                      {'mode': mode, 'tmdb_id': str(item['id']), 'type': content_type, 'title': title}, 
                      folder=(content_type == 'tv'), thumb=poster, plot=item.get('overview'), is_playable=(content_type == 'movie'))
    
    if page < data.get('total_pages', 1):
        add_directory("➡️ Pagina Următoare", {'mode': 'list', 'type': content_type, 'page': str(page + 1), 'query': query})
    xbmcplugin.endOfDirectory(HANDLE)
...
def show_seasons(tmdb_id):
    ...
    for s in data.get('seasons', []):
        ...
        add_directory(f"Sezonul {s['season_number']}", {'mode': 'episodes', 'tmdb_id': tmdb_id, 'season': str(s['season_number']), 'tv_show_title': data.get('name')}, thumb=IMG_BASE + (data.get('poster_path', '')))
    xbmcplugin.endOfDirectory(HANDLE)
...
def list_episodes(tmdb_id, season_num, tv_show_title):
    ...
    for ep in data.get('episodes', []):
        params = {'mode': 'play', 'tmdb_id': tmdb_id, 'type': 'tv', 'season': season_num, 'episode': str(ep['episode_number']), 'title': tv_show_title}
        add_directory(f"E{ep['episode_number']} - {ep.get('name')}", params, folder=False, thumb=IMG_BASE + (ep.get('still_path') or ''), is_playable=True)
    xbmcplugin.endOfDirectory(HANDLE)
...
def router():
    ...
    elif mode == 'list': list_content(params.get('type'), params.get('page', 1), params.get('query'))
    elif mode == 'details': show_seasons(params.get('tmdb_id'))
    elif mode == 'episodes': list_episodes(params.get('tmdb_id'), params.get('season'), params.get('tv_show_title'))
    elif mode == 'play': play_with_subtitles(params)
    elif mode == 'search':
        choice = xbmcgui.Dialog().select('Căutare', ['🎬 Filme', '📺 Seriale'])
        if choice != -1:
            kb = xbmc.Keyboard('', 'Căutare...')
            kb.doModal()
            if kb.isConfirmed(): list_content(['movie', 'tv'][choice], 1, kb.getText())
