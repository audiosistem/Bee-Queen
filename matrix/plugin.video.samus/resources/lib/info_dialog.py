...
class VideoInfoDialog(xbmcgui.WindowXMLDialog):
    ...
    def __init__(self, *_args, **_kwargs):
        ...
        self.play_action = None   # 'play' | 'seasons' | 'trailer'
    ...
    def _populate_cast(self, raw_cast):
        try:
            ...
            for actor in raw_cast[:20]:
                ...
                li = xbmcgui.ListItem(label=name, label2=character)
    ...
    def onClick(self, controlId):
        if controlId == CAST_PANEL:
            ...
            tid = self._item.get('id', '')
            mt  = 'tv' if self._media_type in ('tv', 'tvshow') else 'movie'
            self.close()
            xbmc.executebuiltin(
                f'RunPlugin(plugin://plugin.video.samus'
                f'?action=play_trailer&tmdb_id={tid}&media_type={mt})'
            )
