...
class TraktPlayer(xbmc.Player):
    ...
    def _start_scrobble(self):
        if not self.trakt.access_token:
            xbmc.log("[VeziAici-Trakt] No access token, skipping scrobble.", xbmc.LOGINFO)
            return
        ...
        try:
            ...
            if not title:
                xbmc.log("[VeziAici-Trakt] No title found (Property or Player), skipping.", xbmc.LOGWARNING)
                return
            ...
            match = re.search(r'(.*?)\s*[-–—]\s*Sezonul\s*(\d+)\s*[-–—]\s*Episodul\s*(\d+)', title, re.IGNORECASE)
            ...
            if match:
                ...
                xbmc.log(f"[VeziAici-Trakt] Parsed as Episode: Show='{clean_show_name}', S={season}, E={episode}", xbmc.LOGINFO)
            else:
                ...
                xbmc.log(f"[VeziAici-Trakt] Parsed as Movie/Unknown: '{clean_show_name}'", xbmc.LOGINFO)
            ...
            if is_episode:
                ...
                xbmc.log(f"[VeziAici-Trakt] Show Search Results for '{clean_show_name}': {len(results)} found", xbmc.LOGINFO)
                
                if results:
                    show_data = results[0]['show']
                    show_slug = show_data['ids']['slug']
                    show_title = show_data['title']
                    xbmc.log(f"[VeziAici-Trakt] Matched Show: '{show_title}' ({show_slug})", xbmc.LOGINFO)
                    
                    # Get precise Episode ID
                    episode_data = self.trakt.get_episode_summary(show_slug, season, episode)
                    
                    if episode_data:
                        self.scrobble_data['ids'] = episode_data['ids']
                        self.trakt.scrobble('start', 'episode', episode_data['ids'], progress=0)
                        xbmc.log(f"[VeziAici-Trakt] Scrobble START SUCCESS: {show_title} S{season}E{episode}", xbmc.LOGINFO)
                        xbmcgui.Dialog().notification("Trakt", f"Vizionezi: {show_title} {season}x{episode}", xbmcgui.NOTIFICATION_INFO)
                    else:
                        xbmc.log(f"[VeziAici-Trakt] Episode not found on Trakt API for S{season}E{episode}", xbmc.LOGWARNING)
                else:
                    xbmc.log(f"[VeziAici-Trakt] Show not found on Trakt.", xbmc.LOGWARNING)

            else:
                # Movie Search
                results = self.trakt.search_movie(clean_show_name)
                xbmc.log(f"[VeziAici-Trakt] Movie Search Results for '{clean_show_name}': {len(results)} found", xbmc.LOGINFO)
                ...
                if results:
                    ...
                    xbmc.log(f"[VeziAici-Trakt] Scrobble START SUCCESS: Movie '{movie_data['title']}'", xbmc.LOGINFO)
                    xbmcgui.Dialog().notification("Trakt", f"Vizionezi: {movie_data['title']}", xbmcgui.NOTIFICATION_INFO)
                else:
                    xbmc.log(f"[VeziAici-Trakt] Movie not found on Trakt.", xbmc.LOGWARNING)

        except Exception as e:
            xbmc.log(f"[VeziAici-Trakt] CRITICAL ERROR in onAVStarted: {e}", xbmc.LOGERROR)

        except Exception as e:
            xbmc.log(f"[VeziAici-Trakt] Error in onAVStarted: {e}", xbmc.LOGERROR)
    ...
    def _stop_scrobble(self):
        ...
        try:
            ...
            self.trakt.scrobble(action, item_type, self.scrobble_data['ids'], progress=progress)
            xbmc.log(f"[VeziAici-Trakt] Scrobble {action.upper()} {item_type}", xbmc.LOGINFO)
