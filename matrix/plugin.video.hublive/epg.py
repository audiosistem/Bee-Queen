...
def format_epg_tooltip(items: List[Dict[str, Any]]) -> str:
    """
    Tooltip contains the next 5 programs (starting with current or next upcoming).
    Returns plain text formatted for Kodi (no HTML).
    """
    if not items:
        return "No EPG."
    ...
    # Find current or next upcoming program index
    start_index = 0
    ...
    # Get up to 5 programs starting from current/next
    programs_to_show = items[start_index:start_index + 5]
    ...
    for idx, it in enumerate(programs_to_show):
        ...
        if sd and ed and sd <= now < ed:
            is_current = True
            header = f"[NOW] {name} {start_s} – {end_s}"
        else:
            header = f"{name} {start_s} – {end_s}"

        if date_s and idx == 0:  # Only show date for first item
            header += f" ({date_s})"
        ...
        # Add separator between programs (except after last one)
        if idx < len(programs_to_show) - 1:
...
class EpgManager(threading.Thread):
    """
    ...
    def __init__(
        ...
        connect_timeout: float = 0.6,   # faster connect fail
                read_timeout: float = 10.0,       # don't hang long on slow portals
        ...
        num_workers: int = 10           # NEW: number of parallel workers
    ):
        super().__init__()
    ...
    def _epg_endpoints(self) -> list:
        ...
        Stalker: /portal.php (standard endpoint)
        Generic: /portal.php
        """
        base = self.base_url.rstrip("/")
        # Use /portal.php for both Stalker and generic modes
        ...
        with self._cache_lock:
            ...
            if hit:
                ...
                if fresh and len(cached_full) >= req_size:
                    self.callback(key, cached_full[:req_size])
                    return
    ...
    def _process_one_channel(self, key: str, channel: Dict[str, Any], req_size: int):
        ...
        self.callback(key, normalized_full[:req_size])

    def run(self):
        """Main worker loop with thread pool for parallel processing."""
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            while not self._stop:
                ...
                try:
                    ...
                    future = executor.submit(self._process_one_channel, key, channel, req_size)
                    futures[future] = key
                except queue.Empty:
                    pass
                ...
                for f in done_futures:
                    try:
                        ...
                        logging.warning(f"[EPG] Future failed for key={futures[f]}: {e}")
                    del futures[f]

                # Small sleep to prevent busy loop
                if not self._queue.qsize() and not futures:
                    time.sleep(0.05)
    ...
    def _choose_channel_id(self, ch: Dict[str, Any]) -> Optional[str]:
        ...
        """
        # Priority order: stream_id first for this portal
        for k in ("stream_id", "ch_id", "id", "number", "channel_id", "cmd"):
            v = ch.get(k)
            if v is None:
                continue
            if isinstance(v, (int, str)):
                s = str(v).strip()
                if s:
                    # Return as-is, don't strip non-digits
                    return s
        return None
    ...
    def _channel_key(self, channel):
        ...
        try:
            if isinstance(channel, dict):
                # Priority order: stream_id first to match main.py usage
                for k in ("stream_id", "ch_id", "id", "cid", "ch", "number", "cmd", "name"):
                    v = channel.get(k)
                    if v is not None and str(v).strip():
                        return str(v)
