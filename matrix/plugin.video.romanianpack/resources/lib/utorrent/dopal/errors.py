...
class DopalError(Exception):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) + len(kwargs) > 3:
            raise ValueError("DopalError.__init__ takes at most 3 arguments - %s positional argument(s) given, %s keyword argument(s) given" % (
            len(args), len(kwargs)))
...
class MethodArgumentWrapError(AzMethodError):
    ...
    def __str__(self):
        ...
        error_data = [(str(method_data), str(error.__class__.__name__), str(error)) for (method_data, error) in
