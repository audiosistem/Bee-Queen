...
class HTTP:
    def __init__(self):
        if py3: self._dirname = xbmcvfs.translatePath('special://temp')  # .decode('utf-8').encode('cp1251')
    ...
    def _fetch(self):
        params = {} if self.request.params is None else (self.request.params.encode('utf-8') if py3 else self.request.params)
        ...
        try:
            try: 
                headiter = self.request.headers.iteritems()
            except: 
                headiter = self.request.headers.items()
            for key, value in headiter:
        ...
        if self.request.auth_username and self.request.auth_password:
            if py3:
                req.add_header('Authorization', 'Basic %s' % base64.encodestring(
                    ':'.join([self.request.auth_username, self.request.auth_password]).encode()).decode().strip())
            else:
                req.add_header('Authorization', 'Basic %s' % base64.encodestring(
        ...
        try:
            ...
            if py3: 
                self.response.headers = {k.lower(): v for k, v in dict(self.con.info()).items()}
            else:
    ...
    def _upload(self, upload, params):
        ...
        if params:
            try: paramiter = params.iteritems()
            except: paramiter = params.items()
            for name, value in paramiter:
        ...
        for obj in upload:
            ...
            try:
                if py3: body = open(obj['body'], 'rb').read()
                else: body = obj['body'].read()
...
class HTTPRequest:
    ...
    def __repr__(self):
        try: dictiter = self.__dict__.iteritems()
        except: dictiter = self.__dict__.items()
        return '%s(%s)' % (self.__class__.__name__, ','.join('%s=%r' % i for i in dictiter))
...
class HTTPResponse:
    ...
    def __repr__(self):
        ...
        args = ','.join('%s=%r' % i for i in dictiter if i[0] != 'body')
        if self.body:
            args += ',body=<data>'
        else:
            args += ',body=None'
...
class UTorrent:
    ...
    def dirid(self, dirname):
        if __settings__.getSetting("torrent_save") == '0':
            try: dirid = self.listdirs()[1].index(dirname)
            except: dirid = 0
        else:
    ...
    def add(self, torrent, dirname, ids=None, id=None):
        if py3:
            temppath = os.path.join(self.http._dirname, 'temp.torrent')
            with open(temppath, "wb") as tempfile:
                tempfile.write(base64.b64decode(torrent))
            body = temppath
            dirid = self.dirid(dirname)
            res = self.action('action=add-file&download_dir=%s' % dirid,
                            {'name': 'torrent_file', 'download_dir': dirid,
                            'content-type': 'application/x-bittorrent', 'body': body})
        else:
            dirid = self.dirid(dirname)
            res = self.action('action=add-file&download_dir=' + str(dirid),
                            {'name': 'torrent_file', 'download_dir': str(dirid),
                            'content-type': 'application/x-bittorrent', 'body': base64.b64decode(torrent)})
    ...
    def action(self, uri, upload=None):
        ...
        if py3:
            if upload:
                cookies = cookie.split('=')#dict(cookie)
                cookies = {cookies[0]:cookies[1]}
                params = {'action':'add-file','token': token, 'download-dir': upload.get('download-dir')}
                files = {'torrent_file': open(upload.get('body'), 'rb').read()}
                url = '%s/?%s&token=%s' % (self.url, 'action=add-file', token)
                auth = requests.auth.HTTPBasicAuth(self.login, self.password)
                response = requests.post(self.url, auth=auth, cookies=cookies, params=params, files=files)
                try:
                    obj = response.json()
                except:
                    return None
...
class Transmission:
    ...
    def add(self, torrent, dirname, ids=None, id=None):
        ...
        metainfo = base64.b64encode(base64.b64decode(torrent))
        if py3: metainfo = metainfo.decode('utf-8')
        if self.action({'method': 'torrent-add',
                        'arguments': {'download-dir': dirname, 'metainfo': metainfo}}) is None:
            return None
        #else:
            #if self.action({'method': 'torrent-add', 'arguments': {'download-dir': dirname, 'filename': base64.b64decode(torrent), 'files-unwanted': ids, 'files-wanted' : id}}) is None:
                #return None
        #return True
...
class qBittorrent:
    ...
    def list(self):
        ...
        if len(obj) > 0:
            for r in obj:
                ...
                flist = self.action('/torrents/properties?hash='+r['hash'])
                if len(flist) > 1: add['dir'] = os.path.join(r['save_path'], r['name'])
                res.append(add)
        return res

    def listdirs(self):
        obj = self.action('/app/preferences')
    ...
    def listfiles(self, id):
        obj = self.action('/torrents/files?hash='+id)
    ...
    def add(self, torrent, dirname, ids=None, id=None):
        if not ids and not id:
            upload={'name': 'torrent_file', 'filename': 'and_nothing_else_matters.torrent',
                            'content-type': 'application/x-bittorrent', 'body': base64.b64decode(torrent)}
            res = self.action('/torrents/add', upload=upload)
        else:
            res = self.add_url(base64.b64decode(torrent), dirname)
        if res: return True
    ...
    def setprio_simple(self, id, prio, ind):
        ...
        obj = self.action_post('/torrents/filePrio', params)
        if not obj or ind == None:
            return None
    ...
    def action_simple(self, action, id):
        actions = {'start': ['/torrents/resume',{'hashes':id,}],
                'stop': ['/torrents/pause',{'hashes':id,}],
                'remove': ['/torrents/delete',{'hashes':id}],
                'removedata': ['/torrents/deletePerm',{'hashes':id}]}
        obj = self.action_post(actions[action][0],actions[action][1])
        return True if obj else None

    def get_auth(self):
        params = {"username": self.login, "password": self.password}
        response = self.http.fetch(self.url + '/auth/login', method='POST', params=params, gzip=True)
        if response.error:
            return None
    ...
    def add(self, torrent, dirname, ids=None, id=None):
        if not ids and not id:
            ...
            if self.action({'method': 'core.add_torrent_file',
                            'params': [torrentFile,
                                    base64.b64encode(base64.b64decode(torrent)), {"download_path": dirname}], "id": 3}) is None:
                return None
        else:
            if self.add_url(base64.b64decode(torrent), dirname) is None:
                return None
...
class Vuze:
    ...
    def add(self, torrent, dirname, ids=None, id=None):
        if not ids and not id:
            torrent = self.interface.getTorrentManager().createFromBEncodedData(base64.b64decode(torrent))
            obj = self.interface.getDownloadManager().addDownload(torrent)
            return True if obj else None
        else:
            return self.add_url(base64.b64decode(torrent), dirname)
...
class Download():
    ...
    def add(self, torrent, dirname, ids, id):
        return self.client.add(torrent, dirname, ids, id)
