...
class OSDBServer:
  def __init__( self, *args, **kwargs ):
    ...
    login = self.server.LogIn(__addon__.getSetting( "OSuser" ), __addon__.getSetting( "OSpass" ), "en", "%s_v%s" %('XBMC_Subtitles_Unofficial',__version__))
  ...
  def searchsubtitles( self, item):
    if ( self.osdb_token ) :
      ...
      if len(item['tvshow']) > 0:
        ...
        except: OS_search_string = item['tvshow'].replace(" ","+")
      else:
