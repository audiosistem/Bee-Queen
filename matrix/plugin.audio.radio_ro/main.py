...
def get_api_server():
    selection = _addon.getSetting('api_server_selection')
    if selection == '1':
        return "https://de2.api.radio-browser.info"
    elif selection == '2':
        return "https://fi1.api.radio-browser.info"
    return "https://de1.api.radio-browser.info" # Default (selection '0' or empty)

def get_stations(offset=0, limit=20, order='clickcount', name=None, by_uuid=None, tag=None, override_country=None):
    """
    Fetches the radio station data from the Radio Browser API
    """
    try:
        country_code = override_country if override_country else _addon.getSetting('country_code')
        api_server = get_api_server()

        if by_uuid:
            url = f"{api_server}/json/stations/byuuid/{by_uuid}"
        else:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'Kodi-Radio-RO/1.0.0'})
...
def list_stations(stations, page=1, action='all'):
    ...
    title_prefix = "Radio"
    if 'search' in action: title_prefix = "Results"
    elif 'by_genre' in action: title_prefix = "Genre"
    elif 'by_city' in action: title_prefix = "Local"
    
    if stations:
        xbmcplugin.setPluginCategory(_handle, f"{title_prefix} ({len(stations)} stations found)")
    ...
    for station in stations:
        ...
        list_item.setArt({'thumb': thumb, 'icon': thumb, 'fanart': _addon.getAddonInfo('fanart')})
        list_item.setProperty('IsPlayable', 'true')
        list_item.setProperty('IsLive', 'true')
        url = f"{_base_url}?action=play&station_uuid={station['stationuuid']}"
        commands = []
        commands.append(('Add to Favorites', f'RunPlugin({_base_url}?action=add_favorite&station_uuid={station["stationuuid"]})'))
        commands.append(('Export to Library (.strm)', f'RunPlugin({_base_url}?action=export_strm&station_uuid={station["stationuuid"]})'))
        commands.append(('Play Next', f'RunPlugin({_base_url}?action=queue&station_uuid={station["stationuuid"]}&next=1)'))
        commands.append(('Add to Queue', f'RunPlugin({_base_url}?action=queue&station_uuid={station["stationuuid"]}&next=0)'))
    ...
    if action != 'search' and action != 'favorites':
        if len(stations) == 20:
            ...
            xbmcplugin.addDirectoryItem(_handle, _base_url + f'?action={action}&page={page+1}', li_next, isFolder=True)
        if page > 1:
            ...
            xbmcplugin.addDirectoryItem(_handle, _base_url + f'?action={action}&page={page-1}', li_prev, isFolder=True)
...
def list_genres():
    ...
    try:
        api_server = get_api_server()
        url = f"{api_server}/json/tags/?order=stationcount&reverse=true&hidebroken=true&limit=50"
        req = urllib.request.Request(url, headers={'User-Agent': 'Kodi-Radio-RO/1.0.0'})
        response = urllib.request.urlopen(req)
        ...
        for tag in tags:
            ...
            # 3. Bold genres
            list_item = xbmcgui.ListItem(label=f"[B]{tag_name}[/B] [COLOR gray]({tag.get('stationcount', 0)})[/COLOR]")
            list_item.setArt({'icon': 'DefaultMusicGenres.png', 'thumb': 'DefaultMusicGenres.png'})
...
def main_menu():
    ...
    """
    create_menu_item('[COLOR yellow]Search[/COLOR]', '?action=search', 'DefaultAddonsSearch.png')
    
    favorites = get_favorites()
    if len(favorites) > 0:
        create_menu_item('[COLOR gold]Favorites[/COLOR]', '?action=favorites', 'DefaultTags.png')
        
    create_menu_item('[COLOR lightblue]Most Popular[/COLOR]', '?action=popular&page=1', 'DefaultMusicTop100.png')
    create_menu_item('[COLOR white]All Stations[/COLOR]', '?action=all&page=1', 'DefaultMusicSongs.png')
    create_menu_item('[COLOR pink]Genres / Tags[/COLOR]', '?action=genres', 'DefaultMusicGenres.png')
    create_menu_item('[COLOR green]Local Radios (RO)[/COLOR]', '?action=local_radios', 'DefaultNetwork.png')
    create_menu_item('[COLOR orange]Change Country[/COLOR]', '?action=countries', 'DefaultCountry.png')
    create_menu_item('[COLOR gray]Settings[/COLOR]', '?action=settings', 'DefaultAddonService.png')
...
def list_local_radios():
    ...
    cities = [
        ...
        "Prahova", "Satu Mare", "Salaj", "Sibiu", "Suceava", "Teleorman", "Timis", "Tulcea", "Vaslui", "Valcea", "Vrancea", "Bucuresti"
    ]
    xbmcplugin.setContent(_handle, 'files')
    ...
    for city in sorted(cities):
        ...
        url_action = f"{_base_url}?action=by_city&city={urllib.parse.quote(city)}&page=1"
        xbmcplugin.addDirectoryItem(_handle, url_action, list_item, isFolder=True)
    xbmcplugin.endOfDirectory(_handle)
...
def list_countries():
    ...
    try:
        api_server = get_api_server()
        url = f"{api_server}/json/countries/?order=stationcount&reverse=true&limit=100"
        req = urllib.request.Request(url, headers={'User-Agent': 'Kodi-Radio-RO/1.0.0'})
        response = urllib.request.urlopen(req)
        ...
        for country in countries:
            ...
            list_item = xbmcgui.ListItem(label=f"{name} ({country.get('stationcount', 0)} stations)")
            list_item.setArt({'icon': 'DefaultCountry.png', 'thumb': 'DefaultCountry.png'})
...
def list_favorites():
    ...
    for station in favorites:
        ...
        url = f"{_base_url}?action=play&station_uuid={station['stationuuid']}"
        commands = []
        commands.append(('Remove from Favorites', f'RunPlugin({_base_url}?action=remove_favorite&station_uuid={station["stationuuid"]})'))
        commands.append(('Export to Library (.strm)', f'RunPlugin({_base_url}?action=export_strm&station_uuid={station["stationuuid"]})'))
        commands.append(('Play Next', f'RunPlugin({_base_url}?action=queue&station_uuid={station["stationuuid"]}&next=1)'))
        commands.append(('Add to Queue', f'RunPlugin({_base_url}?action=queue&station_uuid={station["stationuuid"]}&next=0)'))
        list_item.addContextMenuItems(commands)
    ...
    if stations:
        ...
        if url:
            if check_url_alive(url):
                ...
                xbmcgui.Dialog().notification('Radio RO', f'Playing: {station["name"]}', sound=False)
                ...
                if _addon.getSetting('auto_fullscreen') == 'true':
                    xbmc.executebuiltin('AlarmClock(AutoFS,ActivateWindow(visualisation),00:02,silent)')
                
                return
                
    xbmcgui.Dialog().notification('Stream Offline', 'This radio station is currently unreachable.', xbmcgui.NOTIFICATION_ERROR)
...
def export_strm(station_uuid):
    ...
    if stations:
        station = stations[0]
        name = station['name'].replace('/', '_').replace('\\', '_').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
        profile_dir = xbmcvfs.translatePath(_addon.getAddonInfo('profile'))
        ...
        xbmcgui.Dialog().notification('Exported', f'Station saved to Addon Data / strm', time=5000)
...
def router(paramstring):
    ...
    if action == 'all':
        list_stations(get_stations(offset=offset, limit=20, order='name'), page=page, action='all')
    elif action == 'popular':
        ...
        list_stations(stations, page=page, action=f"by_genre&tag={urllib.parse.quote(tag)}")
    elif action == 'local_radios':
        list_local_radios()
    elif action == 'by_city':
        city = urllib.parse.unquote(params.get('city', ''))
        stations = get_stations(offset=offset, limit=20, order='clickcount', name=city, override_country='RO')
        list_stations(stations, page=page, action=f"by_city&city={urllib.parse.quote(city)}")
    elif action == 'countries':
        list_countries()
    elif action == 'set_country':
        set_country(urllib.parse.unquote(params.get('code', '')), urllib.parse.unquote(params.get('name', '')))
    elif action == 'search':
