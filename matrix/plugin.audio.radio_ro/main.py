...
def get_stations(offset=0, limit=20, order='clickcount'):
    ...
    """
    try:
        country_code = _addon.getSetting('country_code')
        
        api_server = "https://de1.api.radio-browser.info" # Default

        if _addon.getSetting('use_de2_server') == 'true':
            api_server = "https://de2.api.radio-browser.info"
        elif _addon.getSetting('use_fi1_server') == 'true':
            api_server = "https://fi1.api.radio-browser.info"
        # If multiple are selected, the order of if/elif determines priority.
        # If none are selected, the default will be used.
...
def list_stations(stations, page=1, action='all'):
    ...
    for station in stations:
        ...
        list_item.setInfo('music', {
            ...
            'album': station.get('country', ''), # Using album for country
            'comment': station.get('language', ''), # Using comment for language
    ...
    # Pagination
    if action == 'all' or action == 'popular':
        if len(stations) == 20:
            xbmcplugin.addDirectoryItem(_handle, _base_url + f'?action={action}&page={page+1}', xbmcgui.ListItem('Next >>'), isFolder=True)
        if page > 1:
            xbmcplugin.addDirectoryItem(_handle, _base_url + f'?action={action}&page={page-1}', xbmcgui.ListItem('<< Previous'), isFolder=True)
...
def main_menu():
    ...
    """
    xbmcplugin.addDirectoryItem(_handle, _base_url + '?action=popular&page=1', xbmcgui.ListItem('Most Popular'), isFolder=True)
    xbmcplugin.addDirectoryItem(_handle, _base_url + '?action=all&page=1', xbmcgui.ListItem('All Stations'), isFolder=True)
    xbmcplugin.addDirectoryItem(_handle, _base_url + '?action=favorites', xbmcgui.ListItem('Favorites'), isFolder=True)
    xbmcplugin.addDirectoryItem(_handle, _base_url + '?action=search', xbmcgui.ListItem('Search'), isFolder=True)
    xbmcplugin.addDirectoryItem(_handle, _base_url + '?action=settings', xbmcgui.ListItem('Settings'), isFolder=True)
    xbmcplugin.endOfDirectory(_handle)
...
def search():
    ...
    if keyboard.isConfirmed():
        ...
        stations = get_stations(limit=1000) # Fetch more stations to get better search results
        search_results = []
        for station in stations:
...
def add_favorite(station_uuid):
    ...
    if station_uuid not in [f['stationuuid'] for f in favorites]:
        ...
        if station:
            ...
            _addon.setSetting('favorites', json.dumps(favorites))
