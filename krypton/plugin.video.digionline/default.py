...
def listCat():
  if(addon.getSetting('api') == 'true'):
    deviceIdFile = os.path.join(xbmc.translatePath(addon.getAddonInfo('profile')), '.deviceId')
...
def listCh(url, idCat):
  if(idCat != None):
    deviceIdFile = os.path.join(xbmc.translatePath(addon.getAddonInfo('profile')), '.deviceId')
    epgFile = os.path.join(xbmc.translatePath(addon.getAddonInfo('profile')), '.epg')
    ...
    if url == None:
      addon_log('Url is None')
    else: 
      addon_log('Url='+url)
    addon_log('deviceId=' + addon.getSetting('deviceId'))
    addon_log('DOSESSV3PRI=' + addon.getSetting('DOSESSV3PRI'))
    digi = Digi(deviceId=addon.getSetting('deviceId'), DOSESSV3PRI=addon.getSetting('DOSESSV3PRI'))
    if digi.siteUrl == None:
      addon_log('siteUrl is None')
    else: 
      addon_log('siteUrl='+digi.siteUrl)
...
try:
  ...
  url=''
try:
  idCat=urllib.unquote_plus(params["idCat"])
except:
  idCat=None
try:
  idCh=urllib.unquote_plus(params["idCh"])
except:
  idCh=None
try:
