            ...
            for title, identifier, filesize in contentListNew:
                ...
                for tname, tvar, tnum in torrvariants:
                    if clickactiontype != tnum:
                        if tnum in ['1','4','0']:
                            continue
                        contextMenu.insert(i, (tname, 'RunPlugin(%s?action=OpenT&Tmode=playmrsp&Turl=%s&Tid=%s&info=%s&site=%s&files=%s&download=true&Tsite=%s)' % (sys.argv[0],quote(url),identifier,quote(info),site,quote(str(files)),site)))
                        i += 1
                listitem.addContextMenuItems(contextMenu, replaceItems=True)
                listings.append(('%s?action=OpenT&Tmode=playmrsp&Turl=%s&Tid=%s&info=%s&site=%s&files=%s&download=%s&Tsite=%s' % (sys.argv[0],quote(url),identifier,quote(info),site,quote(str(files)),('true' if clickactiontype == '3' else 'false'),site), listitem, False))
            p_handle = int(sys.argv[1])
            try:
                xbmcplugin.addSortMethod(p_handle, xbmcplugin.SORT_METHOD_LABEL)
                if hasSize:
            ...
            if get('modify'):
                ...
                if procs_started:
                    for resume_file, proc_started in procs_started:
                        ...
                        popup.append(('Stop and remove files', contextMenustring % ('%s&resume_file=%s' % (quote('%sstopanddelete' % link), quote(resume_file)))))
