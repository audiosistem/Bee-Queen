...
class MaintenanceMenu:
    ...
    def clean_menu(self):
        ...
        if includeall == 'true':
            ...
            includethepromise = 'true' if CONFIG.INCLUDETHEPROMISE == 'true' else 'false'
        ...
        if includevid == 'true':
            ...
            if xbmc.getCondVisibility('System.HasAddon(plugin.video.thepromise)'):
                directory.add_file(
                    '--- Include THE PROMISE: {0}'.format(includethepromise.replace('true', on).replace('false', off)),
                    {'mode': 'togglecache', 'name': 'includethepromise'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
